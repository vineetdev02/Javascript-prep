// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  03_module-pattern-via-closures.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Module pattern via closures
//
...
//
// Runtime rule:
//   an IIFE returns an API that closes over private variables
//
// Practical rule:
//   This is the pre-ES-module/private-field privacy pattern.
//
// Common trap:
//   Returning private mutable state directly.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/03_module-pattern-via-closures.js"
...
const notebook = createInterviewNotebook("Module pattern via closures");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Module pattern via closures");
  steps.push("2. Apply rule: an IIFE returns an API that closes over private variables");
  steps.push("3. Avoid trap: Returning private mutable state directly.");
...
//
// Interview framing:
//   "I care about Module pattern via closures because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Returning private mutable state directly.",
    rule: "an IIFE returns an API that closes over private variables",
    fix: "This is the pre-ES-module/private-field privacy pattern.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Module pattern via closures", answer like this:
//
//   "At a high level, The module pattern uses closures to create private state and public methods.
//    The important runtime rule is: an IIFE returns an API that closes over private variables
//    A common trap is: Returning private mutable state directly.
...
// A1. The module pattern uses closures to create private state and public methods.
//
// Q2. What is the runtime rule?
// A2. an IIFE returns an API that closes over private variables
//
// Q3. What bug does this cause in real projects?
// A3. Returning private mutable state directly.
//
// Q4. How do you avoid the bug?
// A4. This is the pre-ES-module/private-field privacy pattern.
//
...
const summary = {
  topic: "Module pattern via closures",
  keyword: "IIFE module",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Module pattern via closures?
//   Back : The module pattern uses closures to create private state and public methods.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : an IIFE returns an API that closes over private variables
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Returning private mutable state directly.
//
// Flashcard 4:
...
//
// For "Module pattern via closures", practice these levels:
//
// Level 1 — Definition:
//   The module pattern uses closures to create private state and public methods.
//
...
//    In this case, the rule is: an IIFE returns an API that closes over private variables
//    That is why the common bug happens: Returning private mutable state directly."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Module pattern via closures. Predict the output and
...
//   3. The exact rule: an IIFE returns an API that closes over private variables
//   4. The output or state change.
//   5. The bug risk: Returning private mutable state directly.
//
...
//   This is the pre-ES-module/private-field privacy pattern.
//
// NEXT TOPIC -> 04_memory-leaks-via-closures.js
