// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  08_finalizationregistry.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: FinalizationRegistry
//
...
// FinalizationRegistry schedules cleanup after an object is collected.
//
...
//
// Runtime rule:
//   cleanup callback timing is not guaranteed
//
// Practical rule:
//   Use only for best-effort cleanup, not business logic.
//
// Common trap:
//   Depending on finalizers for required resource release.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/08_finalizationregistry.js"
...
const notebook = createInterviewNotebook("FinalizationRegistry");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: FinalizationRegistry");
  steps.push("2. Apply rule: cleanup callback timing is not guaranteed");
  steps.push("3. Avoid trap: Depending on finalizers for required resource release.");
...
//
// Interview framing:
//   "I care about FinalizationRegistry because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Depending on finalizers for required resource release.",
    rule: "cleanup callback timing is not guaranteed",
    fix: "Use only for best-effort cleanup, not business logic.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "FinalizationRegistry", answer like this:
//
//   "At a high level, FinalizationRegistry schedules cleanup after an object is collected.
//    The important runtime rule is: cleanup callback timing is not guaranteed
//    A common trap is: Depending on finalizers for required resource release.
...
// A1. FinalizationRegistry schedules cleanup after an object is collected.
//
// Q2. What is the runtime rule?
// A2. cleanup callback timing is not guaranteed
//
// Q3. What bug does this cause in real projects?
// A3. Depending on finalizers for required resource release.
//
// Q4. How do you avoid the bug?
// A4. Use only for best-effort cleanup, not business logic.
//
...
//
// Flashcard 1:
//   Front: What is FinalizationRegistry?
//   Back : FinalizationRegistry schedules cleanup after an object is collected.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : cleanup callback timing is not guaranteed
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Depending on finalizers for required resource release.
//
// Flashcard 4:
...
//
// For "FinalizationRegistry", practice these levels:
//
// Level 1 — Definition:
//   FinalizationRegistry schedules cleanup after an object is collected.
//
...
//    In this case, the rule is: cleanup callback timing is not guaranteed
//    That is why the common bug happens: Depending on finalizers for required resource release."
//
...
//
// Practice prompt:
//   "Here is a code snippet using FinalizationRegistry. Predict the output and
...
//   3. The exact rule: cleanup callback timing is not guaranteed
//   4. The output or state change.
//   5. The bug risk: Depending on finalizers for required resource release.
//
...
//   Use only for best-effort cleanup, not business logic.
//
// NEXT TOPIC -> 09_memoization-implementation.js
