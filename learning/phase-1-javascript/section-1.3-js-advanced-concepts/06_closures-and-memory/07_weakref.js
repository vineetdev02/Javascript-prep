// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  07_weakref.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: WeakRef
//
...
//
// WeakRef:
// WeakRef points at an object without keeping it alive.
//
...
//
// Practical rule:
//   WeakRef is advanced and rare; do not build core correctness on it.
//
// Common trap:
//   Expecting deterministic garbage collection timing.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/07_weakref.js"
...
const notebook = createInterviewNotebook("WeakRef");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: WeakRef");
  steps.push("2. Apply rule: `deref()` may return the object or undefined");
  steps.push("3. Avoid trap: Expecting deterministic garbage collection timing.");
...
//
// Interview framing:
//   "I care about WeakRef because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Expecting deterministic garbage collection timing.",
    rule: "`deref()` may return the object or undefined",
    fix: "WeakRef is advanced and rare; do not build core correctness on it.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "WeakRef", answer like this:
//
//   "At a high level, WeakRef points at an object without keeping it alive.
//    The important runtime rule is: `deref()` may return the object or undefined
//    A common trap is: Expecting deterministic garbage collection timing.
...
// A1. WeakRef points at an object without keeping it alive.
//
// Q2. What is the runtime rule?
// A2. `deref()` may return the object or undefined
//
// Q3. What bug does this cause in real projects?
// A3. Expecting deterministic garbage collection timing.
//
// Q4. How do you avoid the bug?
// A4. WeakRef is advanced and rare; do not build core correctness on it.
//
...
//
// Flashcard 1:
//   Front: What is WeakRef?
//   Back : WeakRef points at an object without keeping it alive.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `deref()` may return the object or undefined
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Expecting deterministic garbage collection timing.
//
// Flashcard 4:
...
//
// For "WeakRef", practice these levels:
//
// Level 1 — Definition:
//   WeakRef points at an object without keeping it alive.
//
...
//    In this case, the rule is: `deref()` may return the object or undefined
//    That is why the common bug happens: Expecting deterministic garbage collection timing."
//
...
//
// Practice prompt:
//   "Here is a code snippet using WeakRef. Predict the output and
...
//   3. The exact rule: `deref()` may return the object or undefined
//   4. The output or state change.
//   5. The bug risk: Expecting deterministic garbage collection timing.
//
...
//   WeakRef is advanced and rare; do not build core correctness on it.
//
// NEXT TOPIC -> 08_finalizationregistry.js
