// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  02_practical-closure-examples.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Practical closure examples
//
...
// Closures power factories, configuration, counters, and callbacks.
//
...
//
// Runtime rule:
//   returned functions keep access to factory parameters
//
// Practical rule:
//   Show a factory example in interviews.
//
// Common trap:
//   Sharing one closure state accidentally across users/requests.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/02_practical-closure-examples.js"
...
const notebook = createInterviewNotebook("Practical closure examples");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Practical closure examples");
  steps.push("2. Apply rule: returned functions keep access to factory parameters");
  steps.push("3. Avoid trap: Sharing one closure state accidentally across users/requests.");
...
//
// Interview framing:
//   "I care about Practical closure examples because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Sharing one closure state accidentally across users/requests.",
    rule: "returned functions keep access to factory parameters",
    fix: "Show a factory example in interviews.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Practical closure examples", answer like this:
//
//   "At a high level, Closures power factories, configuration, counters, and callbacks.
//    The important runtime rule is: returned functions keep access to factory parameters
//    A common trap is: Sharing one closure state accidentally across users/requests.
...
// A1. Closures power factories, configuration, counters, and callbacks.
//
// Q2. What is the runtime rule?
// A2. returned functions keep access to factory parameters
//
// Q3. What bug does this cause in real projects?
// A3. Sharing one closure state accidentally across users/requests.
//
// Q4. How do you avoid the bug?
// A4. Show a factory example in interviews.
//
...
const summary = {
  topic: "Practical closure examples",
  keyword: "stateful function",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Practical closure examples?
//   Back : Closures power factories, configuration, counters, and callbacks.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : returned functions keep access to factory parameters
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Sharing one closure state accidentally across users/requests.
//
// Flashcard 4:
...
//
// For "Practical closure examples", practice these levels:
//
// Level 1 — Definition:
//   Closures power factories, configuration, counters, and callbacks.
//
...
//    In this case, the rule is: returned functions keep access to factory parameters
//    That is why the common bug happens: Sharing one closure state accidentally across users/requests."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Practical closure examples. Predict the output and
...
//   3. The exact rule: returned functions keep access to factory parameters
//   4. The output or state change.
//   5. The bug risk: Sharing one closure state accidentally across users/requests.
//
...
//   Show a factory example in interviews.
//
// NEXT TOPIC -> 03_module-pattern-via-closures.js
