// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  05_garbage-collection.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Garbage collection
//
...
//
// Runtime rule:
//   roots keep references alive; unreachable graphs can be collected
//
// Practical rule:
//   Think in references, not manual free calls.
//
// Common trap:
//   Setting one reference to null while others still point to the object.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/05_garbage-collection.js"
...
const notebook = createInterviewNotebook("Garbage collection");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Garbage collection");
  steps.push("2. Apply rule: roots keep references alive; unreachable graphs can be collected");
  steps.push("3. Avoid trap: Setting one reference to null while others still point to the object.");
...
//
// Interview framing:
//   "I care about Garbage collection because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Setting one reference to null while others still point to the object.",
    rule: "roots keep references alive; unreachable graphs can be collected",
    fix: "Think in references, not manual free calls.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Garbage collection", answer like this:
//
//   "At a high level, Garbage collection frees objects that are no longer reachable.
//    The important runtime rule is: roots keep references alive; unreachable graphs can be collected
//    A common trap is: Setting one reference to null while others still point to the object.
...
// A1. Garbage collection frees objects that are no longer reachable.
//
// Q2. What is the runtime rule?
// A2. roots keep references alive; unreachable graphs can be collected
//
// Q3. What bug does this cause in real projects?
// A3. Setting one reference to null while others still point to the object.
//
// Q4. How do you avoid the bug?
// A4. Think in references, not manual free calls.
//
...
//   Back : Garbage collection frees objects that are no longer reachable.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : roots keep references alive; unreachable graphs can be collected
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Setting one reference to null while others still point to the object.
//
// Flashcard 4:
...
//
// For "Garbage collection", practice these levels:
//
// Level 1 — Definition:
//   Garbage collection frees objects that are no longer reachable.
//
...
//    In this case, the rule is: roots keep references alive; unreachable graphs can be collected
//    That is why the common bug happens: Setting one reference to null while others still point to the object."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Garbage collection. Predict the output and
...
//   3. The exact rule: roots keep references alive; unreachable graphs can be collected
//   4. The output or state change.
//   5. The bug risk: Setting one reference to null while others still point to the object.
//
...
//   Think in references, not manual free calls.
//
// NEXT TOPIC -> 06_weakmap-weakset.js
