// ╔══════════════════════════════════════════════════════════════════╗
// ║   Closures & Memory  →  06_weakmap-weakset.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: WeakMap / WeakSet
//
...
//
// WeakMap / WeakSet:
// WeakMap and WeakSet hold object keys weakly.
//
...
//
// Runtime rule:
//   weak keys do not prevent garbage collection
//
// Practical rule:
//   Use WeakMap for private object metadata.
//
// Common trap:
//   Trying to iterate WeakMap or use primitive keys.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/06_closures-and-memory/06_weakmap-weakset.js"
...
const notebook = createInterviewNotebook("WeakMap / WeakSet");
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: WeakMap / WeakSet");
  steps.push("2. Apply rule: weak keys do not prevent garbage collection");
  steps.push("3. Avoid trap: Trying to iterate WeakMap or use primitive keys.");
...
//
// Interview framing:
//   "I care about WeakMap / WeakSet because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Trying to iterate WeakMap or use primitive keys.",
    rule: "weak keys do not prevent garbage collection",
    fix: "Use WeakMap for private object metadata.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "WeakMap / WeakSet", answer like this:
//
//   "At a high level, WeakMap and WeakSet hold object keys weakly.
//    The important runtime rule is: weak keys do not prevent garbage collection
//    A common trap is: Trying to iterate WeakMap or use primitive keys.
...
// A1. WeakMap and WeakSet hold object keys weakly.
//
// Q2. What is the runtime rule?
// A2. weak keys do not prevent garbage collection
//
// Q3. What bug does this cause in real projects?
// A3. Trying to iterate WeakMap or use primitive keys.
//
// Q4. How do you avoid the bug?
// A4. Use WeakMap for private object metadata.
//
...
//
// Flashcard 1:
//   Front: What is WeakMap / WeakSet?
//   Back : WeakMap and WeakSet hold object keys weakly.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : weak keys do not prevent garbage collection
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Trying to iterate WeakMap or use primitive keys.
//
// Flashcard 4:
...
//
// For "WeakMap / WeakSet", practice these levels:
//
// Level 1 — Definition:
//   WeakMap and WeakSet hold object keys weakly.
//
...
//    In this case, the rule is: weak keys do not prevent garbage collection
//    That is why the common bug happens: Trying to iterate WeakMap or use primitive keys."
//
...
//
// Practice prompt:
//   "Here is a code snippet using WeakMap / WeakSet. Predict the output and
...
//   3. The exact rule: weak keys do not prevent garbage collection
//   4. The output or state change.
//   5. The bug risk: Trying to iterate WeakMap or use primitive keys.
//
