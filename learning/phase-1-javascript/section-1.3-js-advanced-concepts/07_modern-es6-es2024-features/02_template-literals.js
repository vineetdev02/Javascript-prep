// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  02_template-literals.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Template literals
//
...
//
// Template literals:
// Template literals support interpolation, multiline strings, and tags.
//
...
//
// Runtime rule:
//   `${expr}` inserts evaluated expressions
//
// Practical rule:
//   Use them for readable string composition.
//
// Common trap:
//   Injecting unescaped user content into HTML strings.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/02_template-literals.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Template literals");
  steps.push("2. Apply rule: `${expr}` inserts evaluated expressions");
  steps.push("3. Avoid trap: Injecting unescaped user content into HTML strings.");
...
//
// Interview framing:
//   "I care about Template literals because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Injecting unescaped user content into HTML strings.",
    rule: "`${expr}` inserts evaluated expressions",
    fix: "Use them for readable string composition.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Template literals", answer like this:
//
//   "At a high level, Template literals support interpolation, multiline strings, and tags.
//    The important runtime rule is: `${expr}` inserts evaluated expressions
//    A common trap is: Injecting unescaped user content into HTML strings.
...
// A1. Template literals support interpolation, multiline strings, and tags.
//
// Q2. What is the runtime rule?
// A2. `${expr}` inserts evaluated expressions
//
// Q3. What bug does this cause in real projects?
// A3. Injecting unescaped user content into HTML strings.
//
// Q4. How do you avoid the bug?
// A4. Use them for readable string composition.
//
...
//
// Flashcard 1:
//   Front: What is Template literals?
//   Back : Template literals support interpolation, multiline strings, and tags.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `${expr}` inserts evaluated expressions
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Injecting unescaped user content into HTML strings.
//
// Flashcard 4:
...
//
// For "Template literals", practice these levels:
//
// Level 1 — Definition:
//   Template literals support interpolation, multiline strings, and tags.
//
...
//    In this case, the rule is: `${expr}` inserts evaluated expressions
//    That is why the common bug happens: Injecting unescaped user content into HTML strings."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Template literals. Predict the output and
...
//   3. The exact rule: `${expr}` inserts evaluated expressions
//   4. The output or state change.
//   5. The bug risk: Injecting unescaped user content into HTML strings.
//
