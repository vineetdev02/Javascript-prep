// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  09_nullish-coalescing.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Nullish coalescing (??)
//
...
//
// Nullish coalescing (??):
// Nullish coalescing defaults only on null or undefined.
//
...
//
// Runtime rule:
//   `0 ?? 10` is 0, but `0 || 10` is 10
//
// Practical rule:
//   Use ?? when 0, false, or empty string are valid.
//
// Common trap:
//   Using || for defaults when falsy values are meaningful.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/09_nullish-coalescing.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Nullish coalescing (??)");
  steps.push("2. Apply rule: `0 ?? 10` is 0, but `0 || 10` is 10");
  steps.push("3. Avoid trap: Using || for defaults when falsy values are meaningful.");
...
//
// Interview framing:
//   "I care about Nullish coalescing (??) because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Using || for defaults when falsy values are meaningful.",
    rule: "`0 ?? 10` is 0, but `0 || 10` is 10",
    fix: "Use ?? when 0, false, or empty string are valid.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Nullish coalescing (??)", answer like this:
//
//   "At a high level, Nullish coalescing defaults only on null or undefined.
//    The important runtime rule is: `0 ?? 10` is 0, but `0 || 10` is 10
//    A common trap is: Using || for defaults when falsy values are meaningful.
...
// A1. Nullish coalescing defaults only on null or undefined.
//
// Q2. What is the runtime rule?
// A2. `0 ?? 10` is 0, but `0 || 10` is 10
//
// Q3. What bug does this cause in real projects?
// A3. Using || for defaults when falsy values are meaningful.
//
// Q4. How do you avoid the bug?
// A4. Use ?? when 0, false, or empty string are valid.
//
...
//
// Flashcard 1:
//   Front: What is Nullish coalescing (??)?
//   Back : Nullish coalescing defaults only on null or undefined.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `0 ?? 10` is 0, but `0 || 10` is 10
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Using || for defaults when falsy values are meaningful.
//
// Flashcard 4:
...
//
// For "Nullish coalescing (??)", practice these levels:
//
// Level 1 — Definition:
//   Nullish coalescing defaults only on null or undefined.
//
...
//    In this case, the rule is: `0 ?? 10` is 0, but `0 || 10` is 10
//    That is why the common bug happens: Using || for defaults when falsy values are meaningful."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Nullish coalescing (??). Predict the output and
...
//   3. The exact rule: `0 ?? 10` is 0, but `0 || 10` is 10
//   4. The output or state change.
//   5. The bug risk: Using || for defaults when falsy values are meaningful.
//
...
//   Use ?? when 0, false, or empty string are valid.
//
// NEXT TOPIC -> 10_logical-assignment.js
