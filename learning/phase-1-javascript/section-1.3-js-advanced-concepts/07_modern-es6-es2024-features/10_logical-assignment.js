// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  10_logical-assignment.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Logical assignment (||=, &&=)
//
...
//
// Logical assignment (||=, &&=):
// Logical assignment combines a logical check with assignment.
//
...
//
// Runtime rule:
//   `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//
// Practical rule:
//   Prefer ??= for safe defaults.
//
// Common trap:
//   Using ||= and overwriting valid 0 or empty string values.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/10_logical-assignment.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Logical assignment (||=, &&=)");
  steps.push("2. Apply rule: `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish");
  steps.push("3. Avoid trap: Using ||= and overwriting valid 0 or empty string values.");
...
//
// Interview framing:
//   "I care about Logical assignment (||=, &&=) because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Using ||= and overwriting valid 0 or empty string values.",
    rule: "`||=` assigns on falsy, `&&=` on truthy, `??=` on nullish",
    fix: "Prefer ??= for safe defaults.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Logical assignment (||=, &&=)", answer like this:
//
//   "At a high level, Logical assignment combines a logical check with assignment.
//    The important runtime rule is: `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//    A common trap is: Using ||= and overwriting valid 0 or empty string values.
...
// A1. Logical assignment combines a logical check with assignment.
//
// Q2. What is the runtime rule?
// A2. `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//
// Q3. What bug does this cause in real projects?
// A3. Using ||= and overwriting valid 0 or empty string values.
//
// Q4. How do you avoid the bug?
// A4. Prefer ??= for safe defaults.
//
...
const summary = {
  topic: "Logical assignment (||=, &&=)",
  keyword: "conditional assignment",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Logical assignment (||=, &&=)?
//   Back : Logical assignment combines a logical check with assignment.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Using ||= and overwriting valid 0 or empty string values.
//
// Flashcard 4:
...
//
// For "Logical assignment (||=, &&=)", practice these levels:
//
// Level 1 — Definition:
//   Logical assignment combines a logical check with assignment.
//
...
//    In this case, the rule is: `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//    That is why the common bug happens: Using ||= and overwriting valid 0 or empty string values."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Logical assignment (||=, &&=). Predict the output and
...
//   3. The exact rule: `||=` assigns on falsy, `&&=` on truthy, `??=` on nullish
//   4. The output or state change.
//   5. The bug risk: Using ||= and overwriting valid 0 or empty string values.
//
...
//   Prefer ??= for safe defaults.
//
// NEXT TOPIC -> 11_array-methods-at-findlast.js
