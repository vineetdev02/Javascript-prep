// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  06_generators-iterators.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Generators & iterators
//
...
// Generators produce lazy iterable sequences with `yield`.
//
...
//
// Runtime rule:
//   calling a generator returns an iterator; code runs on `.next()`
//
// Practical rule:
//   Use for lazy streams, custom iteration, and controlled sequences.
//
// Common trap:
//   Expecting generator body to run immediately.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/06_generators-iterators.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Generators & iterators");
  steps.push("2. Apply rule: calling a generator returns an iterator; code runs on `.next()`");
  steps.push("3. Avoid trap: Expecting generator body to run immediately.");
...
//
// Interview framing:
//   "I care about Generators & iterators because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Expecting generator body to run immediately.",
    rule: "calling a generator returns an iterator; code runs on `.next()`",
    fix: "Use for lazy streams, custom iteration, and controlled sequences.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Generators & iterators", answer like this:
//
//   "At a high level, Generators produce lazy iterable sequences with `yield`.
//    The important runtime rule is: calling a generator returns an iterator; code runs on `.next()`
//    A common trap is: Expecting generator body to run immediately.
...
// A1. Generators produce lazy iterable sequences with `yield`.
//
// Q2. What is the runtime rule?
// A2. calling a generator returns an iterator; code runs on `.next()`
//
// Q3. What bug does this cause in real projects?
// A3. Expecting generator body to run immediately.
//
// Q4. How do you avoid the bug?
// A4. Use for lazy streams, custom iteration, and controlled sequences.
//
...
//
// Flashcard 1:
//   Front: What is Generators & iterators?
//   Back : Generators produce lazy iterable sequences with `yield`.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : calling a generator returns an iterator; code runs on `.next()`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Expecting generator body to run immediately.
//
// Flashcard 4:
...
//
// For "Generators & iterators", practice these levels:
//
// Level 1 — Definition:
//   Generators produce lazy iterable sequences with `yield`.
//
...
//    In this case, the rule is: calling a generator returns an iterator; code runs on `.next()`
//    That is why the common bug happens: Expecting generator body to run immediately."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Generators & iterators. Predict the output and
...
//   3. The exact rule: calling a generator returns an iterator; code runs on `.next()`
//   4. The output or state change.
//   5. The bug risk: Expecting generator body to run immediately.
//
...
//   Use for lazy streams, custom iteration, and controlled sequences.
//
// NEXT TOPIC -> 07_proxy-reflect.js
