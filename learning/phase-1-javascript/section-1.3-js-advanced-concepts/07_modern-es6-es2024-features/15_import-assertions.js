// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  15_import-assertions.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Import assertions
//
...
//
// Import assertions:
// Import assertions/import attributes add metadata to imports.
//
...
//
// Runtime rule:
//   JSON/module resource type can be declared for loaders
//
// Practical rule:
//   Know syntax support varies by runtime version.
//
// Common trap:
//   Assuming every Node/browser supports the same import attribute syntax.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/15_import-assertions.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Import assertions");
  steps.push("2. Apply rule: JSON/module resource type can be declared for loaders");
  steps.push("3. Avoid trap: Assuming every Node/browser supports the same import attribute syntax.");
...
//
// Interview framing:
//   "I care about Import assertions because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Assuming every Node/browser supports the same import attribute syntax.",
    rule: "JSON/module resource type can be declared for loaders",
    fix: "Know syntax support varies by runtime version.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Import assertions", answer like this:
//
//   "At a high level, Import assertions/import attributes add metadata to imports.
//    The important runtime rule is: JSON/module resource type can be declared for loaders
//    A common trap is: Assuming every Node/browser supports the same import attribute syntax.
...
// A1. Import assertions/import attributes add metadata to imports.
//
// Q2. What is the runtime rule?
// A2. JSON/module resource type can be declared for loaders
//
// Q3. What bug does this cause in real projects?
// A3. Assuming every Node/browser supports the same import attribute syntax.
//
// Q4. How do you avoid the bug?
// A4. Know syntax support varies by runtime version.
//
...
//
// Flashcard 1:
//   Front: What is Import assertions?
//   Back : Import assertions/import attributes add metadata to imports.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : JSON/module resource type can be declared for loaders
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Assuming every Node/browser supports the same import attribute syntax.
//
// Flashcard 4:
...
//
// For "Import assertions", practice these levels:
//
// Level 1 — Definition:
//   Import assertions/import attributes add metadata to imports.
//
...
//    In this case, the rule is: JSON/module resource type can be declared for loaders
//    That is why the common bug happens: Assuming every Node/browser supports the same import attribute syntax."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Import assertions. Predict the output and
...
//   3. The exact rule: JSON/module resource type can be declared for loaders
//   4. The output or state change.
//   5. The bug risk: Assuming every Node/browser supports the same import attribute syntax.
//
