// ╔══════════════════════════════════════════════════════════════════╗
// ║   Modern ES6-ES2024 Features  →  14_top-level-await.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: top-level await
//
...
//
// top-level await:
// Top-level await allows awaiting directly in ES modules.
//
...
//
// Runtime rule:
//   module evaluation can pause until awaited promise resolves
//
// Practical rule:
//   Use only in ES modules and understand dependency blocking.
//
// Common trap:
//   Using it in CommonJS scripts.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/07_modern-es6-es2024-features/14_top-level-await.js"
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: top-level await");
  steps.push("2. Apply rule: module evaluation can pause until awaited promise resolves");
  steps.push("3. Avoid trap: Using it in CommonJS scripts.");
...
//
// Interview framing:
//   "I care about top-level await because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Using it in CommonJS scripts.",
    rule: "module evaluation can pause until awaited promise resolves",
    fix: "Use only in ES modules and understand dependency blocking.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "top-level await", answer like this:
//
//   "At a high level, Top-level await allows awaiting directly in ES modules.
//    The important runtime rule is: module evaluation can pause until awaited promise resolves
//    A common trap is: Using it in CommonJS scripts.
...
// A1. Top-level await allows awaiting directly in ES modules.
//
// Q2. What is the runtime rule?
// A2. module evaluation can pause until awaited promise resolves
//
// Q3. What bug does this cause in real projects?
// A3. Using it in CommonJS scripts.
//
// Q4. How do you avoid the bug?
// A4. Use only in ES modules and understand dependency blocking.
//
...
//   Back : Top-level await allows awaiting directly in ES modules.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : module evaluation can pause until awaited promise resolves
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Using it in CommonJS scripts.
//
// Flashcard 4:
...
//
// For "top-level await", practice these levels:
//
// Level 1 — Definition:
//   Top-level await allows awaiting directly in ES modules.
//
...
//    In this case, the rule is: module evaluation can pause until awaited promise resolves
//    That is why the common bug happens: Using it in CommonJS scripts."
//
...
//
// Practice prompt:
//   "Here is a code snippet using top-level await. Predict the output and
...
//   3. The exact rule: module evaluation can pause until awaited promise resolves
//   4. The output or state change.
//   5. The bug risk: Using it in CommonJS scripts.
//
...
//   Use only in ES modules and understand dependency blocking.
//
// NEXT TOPIC -> 15_import-assertions.js
