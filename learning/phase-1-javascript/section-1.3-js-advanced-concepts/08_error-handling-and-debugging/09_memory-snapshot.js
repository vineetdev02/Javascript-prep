// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  09_memory-snapshot.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Memory snapshot
//
...
//
// Memory snapshot:
// Memory snapshots show retained objects and reference paths.
//
...
//
// Runtime rule:
//   leaks come from live references: caches, listeners, intervals, closures
//
// Practical rule:
//   Look for objects that should be gone but remain reachable.
//
// Common trap:
//   Never clearing caches/listeners after UI teardown.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/09_memory-snapshot.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("Memory snapshot failed", "DEMO_FAILURE");
  }
  return "Memory snapshot succeeded";
}
...
try {
  ...
  console.log("cleanup for Memory snapshot");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Memory snapshot");
  steps.push("2. Apply rule: leaks come from live references: caches, listeners, intervals, closures");
  steps.push("3. Avoid trap: Never clearing caches/listeners after UI teardown.");
...
//
// Interview framing:
//   "I care about Memory snapshot because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Never clearing caches/listeners after UI teardown.",
    rule: "leaks come from live references: caches, listeners, intervals, closures",
    fix: "Look for objects that should be gone but remain reachable.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Memory snapshot", answer like this:
//
//   "At a high level, Memory snapshots show retained objects and reference paths.
//    The important runtime rule is: leaks come from live references: caches, listeners, intervals, closures
//    A common trap is: Never clearing caches/listeners after UI teardown.
...
// A1. Memory snapshots show retained objects and reference paths.
//
// Q2. What is the runtime rule?
// A2. leaks come from live references: caches, listeners, intervals, closures
//
// Q3. What bug does this cause in real projects?
// A3. Never clearing caches/listeners after UI teardown.
//
// Q4. How do you avoid the bug?
// A4. Look for objects that should be gone but remain reachable.
//
...
//
// Flashcard 1:
//   Front: What is Memory snapshot?
//   Back : Memory snapshots show retained objects and reference paths.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : leaks come from live references: caches, listeners, intervals, closures
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Never clearing caches/listeners after UI teardown.
//
// Flashcard 4:
...
//
// For "Memory snapshot", practice these levels:
//
// Level 1 — Definition:
//   Memory snapshots show retained objects and reference paths.
//
...
//    In this case, the rule is: leaks come from live references: caches, listeners, intervals, closures
//    That is why the common bug happens: Never clearing caches/listeners after UI teardown."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Memory snapshot. Predict the output and
...
//   3. The exact rule: leaks come from live references: caches, listeners, intervals, closures
//   4. The output or state change.
//   5. The bug risk: Never clearing caches/listeners after UI teardown.
//
