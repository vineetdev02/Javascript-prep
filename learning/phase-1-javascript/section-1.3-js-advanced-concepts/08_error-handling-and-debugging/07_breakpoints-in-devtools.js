// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  07_breakpoints-in-devtools.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Breakpoints in DevTools
//
...
// Breakpoints pause code so you can inspect stack, scope, and values.
//
...
//
// Runtime rule:
//   step over/into/out reveals actual runtime flow
//
// Practical rule:
//   Use breakpoints when logs become messy.
//
// Common trap:
//   Only adding console.log everywhere.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/07_breakpoints-in-devtools.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("Breakpoints in DevTools failed", "DEMO_FAILURE");
  }
  return "Breakpoints in DevTools succeeded";
}
...
try {
  ...
  console.log("cleanup for Breakpoints in DevTools");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Breakpoints in DevTools");
  steps.push("2. Apply rule: step over/into/out reveals actual runtime flow");
  steps.push("3. Avoid trap: Only adding console.log everywhere.");
...
//
// Interview framing:
//   "I care about Breakpoints in DevTools because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Only adding console.log everywhere.",
    rule: "step over/into/out reveals actual runtime flow",
    fix: "Use breakpoints when logs become messy.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Breakpoints in DevTools", answer like this:
//
//   "At a high level, Breakpoints pause code so you can inspect stack, scope, and values.
//    The important runtime rule is: step over/into/out reveals actual runtime flow
//    A common trap is: Only adding console.log everywhere.
...
// A1. Breakpoints pause code so you can inspect stack, scope, and values.
//
// Q2. What is the runtime rule?
// A2. step over/into/out reveals actual runtime flow
//
// Q3. What bug does this cause in real projects?
// A3. Only adding console.log everywhere.
//
// Q4. How do you avoid the bug?
// A4. Use breakpoints when logs become messy.
//
...
const summary = {
  topic: "Breakpoints in DevTools",
  keyword: "debugger pause",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Breakpoints in DevTools?
//   Back : Breakpoints pause code so you can inspect stack, scope, and values.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : step over/into/out reveals actual runtime flow
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Only adding console.log everywhere.
//
// Flashcard 4:
...
//
// For "Breakpoints in DevTools", practice these levels:
//
// Level 1 — Definition:
//   Breakpoints pause code so you can inspect stack, scope, and values.
//
...
//    In this case, the rule is: step over/into/out reveals actual runtime flow
//    That is why the common bug happens: Only adding console.log everywhere."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Breakpoints in DevTools. Predict the output and
...
//   3. The exact rule: step over/into/out reveals actual runtime flow
//   4. The output or state change.
//   5. The bug risk: Only adding console.log everywhere.
//
...
//   Use breakpoints when logs become messy.
//
// NEXT TOPIC -> 08_performance-profiling.js
