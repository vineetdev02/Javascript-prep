// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  03_try-catch-scope.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: try/catch scope
//
...
//
// try/catch scope:
// try/catch creates block scopes; catch parameter is scoped to catch.
//
...
//
// Runtime rule:
//   variables inside catch are not visible outside
//
// Practical rule:
//   Declare outside if you need a value after the block.
//
// Common trap:
//   Trying to use `error` after catch ends.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/03_try-catch-scope.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("try/catch scope failed", "DEMO_FAILURE");
  }
  return "try/catch scope succeeded";
}
...
try {
  ...
  console.log("cleanup for try/catch scope");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: try/catch scope");
  steps.push("2. Apply rule: variables inside catch are not visible outside");
  steps.push("3. Avoid trap: Trying to use `error` after catch ends.");
...
//
// Interview framing:
//   "I care about try/catch scope because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Trying to use `error` after catch ends.",
    rule: "variables inside catch are not visible outside",
    fix: "Declare outside if you need a value after the block.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "try/catch scope", answer like this:
//
//   "At a high level, try/catch creates block scopes; catch parameter is scoped to catch.
//    The important runtime rule is: variables inside catch are not visible outside
//    A common trap is: Trying to use `error` after catch ends.
...
// A1. try/catch creates block scopes; catch parameter is scoped to catch.
//
// Q2. What is the runtime rule?
// A2. variables inside catch are not visible outside
//
// Q3. What bug does this cause in real projects?
// A3. Trying to use `error` after catch ends.
//
// Q4. How do you avoid the bug?
// A4. Declare outside if you need a value after the block.
//
...
const summary = {
  topic: "try/catch scope",
  keyword: "catch binding",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is try/catch scope?
//   Back : try/catch creates block scopes; catch parameter is scoped to catch.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : variables inside catch are not visible outside
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Trying to use `error` after catch ends.
//
// Flashcard 4:
...
//
// For "try/catch scope", practice these levels:
//
// Level 1 — Definition:
//   try/catch creates block scopes; catch parameter is scoped to catch.
//
...
//    In this case, the rule is: variables inside catch are not visible outside
//    That is why the common bug happens: Trying to use `error` after catch ends."
//
...
//
// Practice prompt:
//   "Here is a code snippet using try/catch scope. Predict the output and
...
//   3. The exact rule: variables inside catch are not visible outside
//   4. The output or state change.
//   5. The bug risk: Trying to use `error` after catch ends.
//
...
//   Declare outside if you need a value after the block.
//
// NEXT TOPIC -> 04_unhandled-promise-rejection.js
