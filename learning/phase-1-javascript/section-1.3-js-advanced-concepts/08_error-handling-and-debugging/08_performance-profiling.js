// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  08_performance-profiling.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Performance profiling
//
...
// Performance profiling measures where time is really spent.
//
...
//
// Runtime rule:
//   measure first, optimize the bottleneck second
//
// Practical rule:
//   Use profiler/flamegraph/timing before changing algorithms.
//
// Common trap:
//   Optimizing code that is not actually slow.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/08_performance-profiling.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("Performance profiling failed", "DEMO_FAILURE");
  }
  return "Performance profiling succeeded";
}
...
try {
  ...
  console.log("cleanup for Performance profiling");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Performance profiling");
  steps.push("2. Apply rule: measure first, optimize the bottleneck second");
  steps.push("3. Avoid trap: Optimizing code that is not actually slow.");
...
//
// Interview framing:
//   "I care about Performance profiling because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Optimizing code that is not actually slow.",
    rule: "measure first, optimize the bottleneck second",
    fix: "Use profiler/flamegraph/timing before changing algorithms.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Performance profiling", answer like this:
//
//   "At a high level, Performance profiling measures where time is really spent.
//    The important runtime rule is: measure first, optimize the bottleneck second
//    A common trap is: Optimizing code that is not actually slow.
...
// A1. Performance profiling measures where time is really spent.
//
// Q2. What is the runtime rule?
// A2. measure first, optimize the bottleneck second
//
// Q3. What bug does this cause in real projects?
// A3. Optimizing code that is not actually slow.
//
// Q4. How do you avoid the bug?
// A4. Use profiler/flamegraph/timing before changing algorithms.
//
...
//
// Flashcard 1:
//   Front: What is Performance profiling?
//   Back : Performance profiling measures where time is really spent.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : measure first, optimize the bottleneck second
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Optimizing code that is not actually slow.
//
// Flashcard 4:
...
//
// For "Performance profiling", practice these levels:
//
// Level 1 — Definition:
//   Performance profiling measures where time is really spent.
//
...
//    In this case, the rule is: measure first, optimize the bottleneck second
//    That is why the common bug happens: Optimizing code that is not actually slow."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Performance profiling. Predict the output and
...
//   3. The exact rule: measure first, optimize the bottleneck second
//   4. The output or state change.
//   5. The bug risk: Optimizing code that is not actually slow.
//
...
//   Use profiler/flamegraph/timing before changing algorithms.
//
// NEXT TOPIC -> 09_memory-snapshot.js
