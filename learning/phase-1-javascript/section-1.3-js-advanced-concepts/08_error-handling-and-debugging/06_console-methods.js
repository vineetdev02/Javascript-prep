// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  06_console-methods.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: console methods
//
...
//
// console methods:
// console has tools beyond log: table, time, trace, group, warn, error.
//
...
//
// Runtime rule:
//   choose the console method that matches the debugging job
//
// Practical rule:
//   Use console.time/table/trace to debug faster.
//
// Common trap:
//   Leaving noisy or sensitive logs in production.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/06_console-methods.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("console methods failed", "DEMO_FAILURE");
  }
  return "console methods succeeded";
}
...
try {
  ...
  console.log("cleanup for console methods");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: console methods");
  steps.push("2. Apply rule: choose the console method that matches the debugging job");
  steps.push("3. Avoid trap: Leaving noisy or sensitive logs in production.");
...
//
// Interview framing:
//   "I care about console methods because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Leaving noisy or sensitive logs in production.",
    rule: "choose the console method that matches the debugging job",
    fix: "Use console.time/table/trace to debug faster.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "console methods", answer like this:
//
//   "At a high level, console has tools beyond log: table, time, trace, group, warn, error.
//    The important runtime rule is: choose the console method that matches the debugging job
//    A common trap is: Leaving noisy or sensitive logs in production.
...
// A1. console has tools beyond log: table, time, trace, group, warn, error.
//
// Q2. What is the runtime rule?
// A2. choose the console method that matches the debugging job
//
// Q3. What bug does this cause in real projects?
// A3. Leaving noisy or sensitive logs in production.
//
// Q4. How do you avoid the bug?
// A4. Use console.time/table/trace to debug faster.
//
...
//
// Flashcard 1:
//   Front: What is console methods?
//   Back : console has tools beyond log: table, time, trace, group, warn, error.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : choose the console method that matches the debugging job
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Leaving noisy or sensitive logs in production.
//
// Flashcard 4:
...
//
// For "console methods", practice these levels:
//
// Level 1 — Definition:
//   console has tools beyond log: table, time, trace, group, warn, error.
//
...
//    In this case, the rule is: choose the console method that matches the debugging job
//    That is why the common bug happens: Leaving noisy or sensitive logs in production."
//
...
//
// Practice prompt:
//   "Here is a code snippet using console methods. Predict the output and
...
//   3. The exact rule: choose the console method that matches the debugging job
//   4. The output or state change.
//   5. The bug risk: Leaving noisy or sensitive logs in production.
//
...
//   Use console.time/table/trace to debug faster.
//
// NEXT TOPIC -> 07_breakpoints-in-devtools.js
