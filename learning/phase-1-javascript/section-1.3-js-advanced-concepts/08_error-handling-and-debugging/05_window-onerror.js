// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  05_window-onerror.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: window.onerror
//
...
//
// window.onerror:
// window.onerror is a browser last-resort uncaught error hook.
//
...
//
// Keyword to remember:
//   global browser handler
//
// Runtime rule:
//   script errors go to onerror; promise rejections use unhandledrejection
//
// Practical rule:
//   Use global handlers for logging, not normal control flow.
//
// Common trap:
//   Thinking global handlers replace local error handling.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/08_error-handling-and-debugging/05_window-onerror.js"
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("window.onerror failed", "DEMO_FAILURE");
  }
  return "window.onerror succeeded";
}
...
try {
  ...
  console.log("cleanup for window.onerror");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: window.onerror");
  steps.push("2. Apply rule: script errors go to onerror; promise rejections use unhandledrejection");
  steps.push("3. Avoid trap: Thinking global handlers replace local error handling.");
...
//
// Interview framing:
//   "I care about window.onerror because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Thinking global handlers replace local error handling.",
    rule: "script errors go to onerror; promise rejections use unhandledrejection",
    fix: "Use global handlers for logging, not normal control flow.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "window.onerror", answer like this:
//
//   "At a high level, window.onerror is a browser last-resort uncaught error hook.
//    The important runtime rule is: script errors go to onerror; promise rejections use unhandledrejection
//    A common trap is: Thinking global handlers replace local error handling.
...
// A1. window.onerror is a browser last-resort uncaught error hook.
//
// Q2. What is the runtime rule?
// A2. script errors go to onerror; promise rejections use unhandledrejection
//
// Q3. What bug does this cause in real projects?
// A3. Thinking global handlers replace local error handling.
//
// Q4. How do you avoid the bug?
// A4. Use global handlers for logging, not normal control flow.
//
...
const summary = {
  topic: "window.onerror",
  keyword: "global browser handler",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is window.onerror?
//   Back : window.onerror is a browser last-resort uncaught error hook.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : script errors go to onerror; promise rejections use unhandledrejection
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Thinking global handlers replace local error handling.
//
// Flashcard 4:
...
//
// For "window.onerror", practice these levels:
//
// Level 1 — Definition:
//   window.onerror is a browser last-resort uncaught error hook.
//
...
//    In this case, the rule is: script errors go to onerror; promise rejections use unhandledrejection
//    That is why the common bug happens: Thinking global handlers replace local error handling."
//
...
//
// Practice prompt:
//   "Here is a code snippet using window.onerror. Predict the output and
...
//   3. The exact rule: script errors go to onerror; promise rejections use unhandledrejection
//   4. The output or state change.
//   5. The bug risk: Thinking global handlers replace local error handling.
//
...
//   Use global handlers for logging, not normal control flow.
//
// NEXT TOPIC -> 06_console-methods.js
