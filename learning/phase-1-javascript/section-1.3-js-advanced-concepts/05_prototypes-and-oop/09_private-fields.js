// ╔══════════════════════════════════════════════════════════════════╗
// ║   Prototypes & OOP  →  09_private-fields.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Private fields (#)
//
...
//
// Private fields (#):
// Private fields are real language-enforced private state.
//
...
//
// Runtime rule:
//   `#field` can only be referenced inside the declaring class body
//
// Practical rule:
//   Use private fields for invariants that callers must not mutate.
//
// Common trap:
//   Trying to access `obj.#field` outside the class.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/05_prototypes-and-oop/09_private-fields.js"
...
Engineer.prototype.describe = function() {
  return this.name + " understands Private fields (#)";
};
...
class SeniorEngineer extends Engineer {
  review() {
    return this.name + " can explain private slot in interviews";
  }
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Private fields (#)");
  steps.push("2. Apply rule: `#field` can only be referenced inside the declaring class body");
  steps.push("3. Avoid trap: Trying to access `obj.#field` outside the class.");
...
//
// Interview framing:
//   "I care about Private fields (#) because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Trying to access `obj.#field` outside the class.",
    rule: "`#field` can only be referenced inside the declaring class body",
    fix: "Use private fields for invariants that callers must not mutate.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Private fields (#)", answer like this:
//
//   "At a high level, Private fields are real language-enforced private state.
//    The important runtime rule is: `#field` can only be referenced inside the declaring class body
//    A common trap is: Trying to access `obj.#field` outside the class.
...
// A1. Private fields are real language-enforced private state.
//
// Q2. What is the runtime rule?
// A2. `#field` can only be referenced inside the declaring class body
//
// Q3. What bug does this cause in real projects?
// A3. Trying to access `obj.#field` outside the class.
//
// Q4. How do you avoid the bug?
// A4. Use private fields for invariants that callers must not mutate.
//
...
//
// Flashcard 1:
//   Front: What is Private fields (#)?
//   Back : Private fields are real language-enforced private state.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `#field` can only be referenced inside the declaring class body
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Trying to access `obj.#field` outside the class.
//
// Flashcard 4:
...
//
// For "Private fields (#)", practice these levels:
//
// Level 1 — Definition:
//   Private fields are real language-enforced private state.
//
...
//    In this case, the rule is: `#field` can only be referenced inside the declaring class body
//    That is why the common bug happens: Trying to access `obj.#field` outside the class."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Private fields (#). Predict the output and
...
//   3. The exact rule: `#field` can only be referenced inside the declaring class body
//   4. The output or state change.
//   5. The bug risk: Trying to access `obj.#field` outside the class.
//
...
//   Use private fields for invariants that callers must not mutate.
//
// NEXT TOPIC -> 10_mixin-pattern.js
