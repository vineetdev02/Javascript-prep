// ╔══════════════════════════════════════════════════════════════════╗
// ║   Prototypes & OOP  →  08_static-methods-properties.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Static methods & properties
//
...
//
// Runtime rule:
//   `ClassName.method()` works; `instance.method()` does not for static methods
//
// Practical rule:
//   Use static for factories, counters, parsing helpers, and constants.
//
// Common trap:
//   Trying to access static methods from instances.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/05_prototypes-and-oop/08_static-methods-properties.js"
...
Engineer.prototype.describe = function() {
  return this.name + " understands Static methods & properties";
};
...
class SeniorEngineer extends Engineer {
  review() {
    return this.name + " can explain class-level member in interviews";
  }
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Static methods & properties");
  steps.push("2. Apply rule: `ClassName.method()` works; `instance.method()` does not for static methods");
  steps.push("3. Avoid trap: Trying to access static methods from instances.");
...
//
// Interview framing:
//   "I care about Static methods & properties because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Trying to access static methods from instances.",
    rule: "`ClassName.method()` works; `instance.method()` does not for static methods",
    fix: "Use static for factories, counters, parsing helpers, and constants.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Static methods & properties", answer like this:
//
//   "At a high level, Static members live on the class constructor, not instances.
//    The important runtime rule is: `ClassName.method()` works; `instance.method()` does not for static methods
//    A common trap is: Trying to access static methods from instances.
...
// A1. Static members live on the class constructor, not instances.
//
// Q2. What is the runtime rule?
// A2. `ClassName.method()` works; `instance.method()` does not for static methods
//
// Q3. What bug does this cause in real projects?
// A3. Trying to access static methods from instances.
//
// Q4. How do you avoid the bug?
// A4. Use static for factories, counters, parsing helpers, and constants.
//
...
const summary = {
  topic: "Static methods & properties",
  keyword: "class-level member",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Static methods & properties?
//   Back : Static members live on the class constructor, not instances.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `ClassName.method()` works; `instance.method()` does not for static methods
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Trying to access static methods from instances.
//
// Flashcard 4:
...
//
// For "Static methods & properties", practice these levels:
//
// Level 1 — Definition:
//   Static members live on the class constructor, not instances.
//
...
//    In this case, the rule is: `ClassName.method()` works; `instance.method()` does not for static methods
//    That is why the common bug happens: Trying to access static methods from instances."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Static methods & properties. Predict the output and
...
//   3. The exact rule: `ClassName.method()` works; `instance.method()` does not for static methods
//   4. The output or state change.
//   5. The bug risk: Trying to access static methods from instances.
//
...
//   Use static for factories, counters, parsing helpers, and constants.
//
// NEXT TOPIC -> 09_private-fields.js
