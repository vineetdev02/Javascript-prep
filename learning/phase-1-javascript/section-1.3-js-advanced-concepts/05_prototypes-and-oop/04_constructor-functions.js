// ╔══════════════════════════════════════════════════════════════════╗
// ║   Prototypes & OOP  →  04_constructor-functions.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Constructor functions
//
...
// Constructor functions use `new` to initialize instances.
//
...
//
// Runtime rule:
//   `new` binds `this` and links to `Fn.prototype`
//
// Practical rule:
//   Put shared methods on the prototype.
//
// Common trap:
//   Creating methods inside the constructor for every instance unnecessarily.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/05_prototypes-and-oop/04_constructor-functions.js"
...
Engineer.prototype.describe = function() {
  return this.name + " understands Constructor functions";
};
...
class SeniorEngineer extends Engineer {
  review() {
    return this.name + " can explain constructor in interviews";
  }
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Constructor functions");
  steps.push("2. Apply rule: `new` binds `this` and links to `Fn.prototype`");
  steps.push("3. Avoid trap: Creating methods inside the constructor for every instance unnecessarily.");
...
//
// Interview framing:
//   "I care about Constructor functions because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Creating methods inside the constructor for every instance unnecessarily.",
    rule: "`new` binds `this` and links to `Fn.prototype`",
    fix: "Put shared methods on the prototype.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Constructor functions", answer like this:
//
//   "At a high level, Constructor functions use `new` to initialize instances.
//    The important runtime rule is: `new` binds `this` and links to `Fn.prototype`
//    A common trap is: Creating methods inside the constructor for every instance unnecessarily.
...
// A1. Constructor functions use `new` to initialize instances.
//
// Q2. What is the runtime rule?
// A2. `new` binds `this` and links to `Fn.prototype`
//
// Q3. What bug does this cause in real projects?
// A3. Creating methods inside the constructor for every instance unnecessarily.
//
// Q4. How do you avoid the bug?
// A4. Put shared methods on the prototype.
//
...
//
// Flashcard 1:
//   Front: What is Constructor functions?
//   Back : Constructor functions use `new` to initialize instances.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `new` binds `this` and links to `Fn.prototype`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Creating methods inside the constructor for every instance unnecessarily.
//
// Flashcard 4:
...
//
// For "Constructor functions", practice these levels:
//
// Level 1 — Definition:
//   Constructor functions use `new` to initialize instances.
//
...
//    In this case, the rule is: `new` binds `this` and links to `Fn.prototype`
//    That is why the common bug happens: Creating methods inside the constructor for every instance unnecessarily."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Constructor functions. Predict the output and
...
//   3. The exact rule: `new` binds `this` and links to `Fn.prototype`
//   4. The output or state change.
//   5. The bug risk: Creating methods inside the constructor for every instance unnecessarily.
//
