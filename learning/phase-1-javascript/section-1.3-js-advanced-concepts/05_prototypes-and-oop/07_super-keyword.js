// ╔══════════════════════════════════════════════════════════════════╗
// ║   Prototypes & OOP  →  07_super-keyword.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: super keyword
//
...
//
// Runtime rule:
//   `super()` must run before `this` in subclass constructors
//
// Practical rule:
//   Mention both `super()` and `super.method()`.
//
// Common trap:
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/05_prototypes-and-oop/07_super-keyword.js"
...
Engineer.prototype.describe = function() {
  return this.name + " understands super keyword";
};
...
class SeniorEngineer extends Engineer {
  review() {
    return this.name + " can explain parent access in interviews";
  }
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: super keyword");
  steps.push("2. Apply rule: `super()` must run before `this` in subclass constructors");
  steps.push("3. Avoid trap: Using `this` before `super()`.");
...
//
// Interview framing:
//   "I care about super keyword because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Using `this` before `super()`.",
    rule: "`super()` must run before `this` in subclass constructors",
    fix: "Mention both `super()` and `super.method()`.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "super keyword", answer like this:
//
//   "At a high level, super calls parent constructors and methods.
//    The important runtime rule is: `super()` must run before `this` in subclass constructors
//    A common trap is: Using `this` before `super()`.
...
// A1. super calls parent constructors and methods.
//
// Q2. What is the runtime rule?
// A2. `super()` must run before `this` in subclass constructors
//
// Q3. What bug does this cause in real projects?
// A3. Using `this` before `super()`.
//
// Q4. How do you avoid the bug?
// A4. Mention both `super()` and `super.method()`.
//
...
//
// Flashcard 1:
//   Front: What is super keyword?
//   Back : super calls parent constructors and methods.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `super()` must run before `this` in subclass constructors
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Using `this` before `super()`.
//
// Flashcard 4:
...
//
// For "super keyword", practice these levels:
//
// Level 1 — Definition:
//   super calls parent constructors and methods.
//
...
//    In this case, the rule is: `super()` must run before `this` in subclass constructors
//    That is why the common bug happens: Using `this` before `super()`."
//
...
//
// Practice prompt:
//   "Here is a code snippet using super keyword. Predict the output and
...
//   3. The exact rule: `super()` must run before `this` in subclass constructors
//   4. The output or state change.
//   5. The bug risk: Using `this` before `super()`.
//
...
//   Mention both `super()` and `super.method()`.
//
// NEXT TOPIC -> 08_static-methods-properties.js
