// ╔══════════════════════════════════════════════════════════════════╗
// ║   Prototypes & OOP  →  01_prototype-chain.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Prototype chain
//
...
//
// Prototype chain:
// The prototype chain is JavaScript's property lookup path.
//
...
//
// Runtime rule:
//   own property first, then prototype, then prototype's prototype
//
// Practical rule:
//   Use `hasOwnProperty` or `Object.hasOwn` when ownership matters.
//
// Common trap:
//   Confusing inherited values with own values.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.3-js-advanced-concepts/05_prototypes-and-oop/01_prototype-chain.js"
...
Engineer.prototype.describe = function() {
  return this.name + " understands Prototype chain";
};
...
class SeniorEngineer extends Engineer {
  review() {
    return this.name + " can explain lookup chain in interviews";
  }
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Prototype chain");
  steps.push("2. Apply rule: own property first, then prototype, then prototype's prototype");
  steps.push("3. Avoid trap: Confusing inherited values with own values.");
...
//
// Interview framing:
//   "I care about Prototype chain because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Confusing inherited values with own values.",
    rule: "own property first, then prototype, then prototype's prototype",
    fix: "Use `hasOwnProperty` or `Object.hasOwn` when ownership matters.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Prototype chain", answer like this:
//
//   "At a high level, The prototype chain is JavaScript's property lookup path.
//    The important runtime rule is: own property first, then prototype, then prototype's prototype
//    A common trap is: Confusing inherited values with own values.
...
// A1. The prototype chain is JavaScript's property lookup path.
//
// Q2. What is the runtime rule?
// A2. own property first, then prototype, then prototype's prototype
//
// Q3. What bug does this cause in real projects?
// A3. Confusing inherited values with own values.
//
// Q4. How do you avoid the bug?
// A4. Use `hasOwnProperty` or `Object.hasOwn` when ownership matters.
//
...
//   Back : The prototype chain is JavaScript's property lookup path.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : own property first, then prototype, then prototype's prototype
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Confusing inherited values with own values.
//
// Flashcard 4:
...
//
// For "Prototype chain", practice these levels:
//
// Level 1 — Definition:
//   The prototype chain is JavaScript's property lookup path.
//
...
//    In this case, the rule is: own property first, then prototype, then prototype's prototype
//    That is why the common bug happens: Confusing inherited values with own values."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Prototype chain. Predict the output and
...
//   3. The exact rule: own property first, then prototype, then prototype's prototype
//   4. The output or state change.
//   5. The bug risk: Confusing inherited values with own values.
//
...
//   Use `hasOwnProperty` or `Object.hasOwn` when ownership matters.
//
// NEXT TOPIC -> 02_proto-vs-prototype.js
