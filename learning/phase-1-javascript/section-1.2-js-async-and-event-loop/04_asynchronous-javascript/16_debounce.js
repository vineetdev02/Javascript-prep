// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  16_debounce.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Debounce
//
...
//
// Debounce:
// Debounce waits until calls stop before running a function.
//
...
//
// Practical rule:
//   Use for search input, resize-end, and autosave-after-typing.
//
// Common trap:
//   Using debounce when continuous feedback is required.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/16_debounce.js"

console.log("A sync start");

const job = Promise.resolve("Debounce");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Debounce");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Debounce");
  steps.push("2. Apply rule: each call resets the timer");
  steps.push("3. Avoid trap: Using debounce when continuous feedback is required.");
...
//
// Interview framing:
//   "I care about Debounce because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Using debounce when continuous feedback is required.",
    rule: "each call resets the timer",
    fix: "Use for search input, resize-end, and autosave-after-typing.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Debounce", answer like this:
//
//   "At a high level, Debounce waits until calls stop before running a function.
//    The important runtime rule is: each call resets the timer
//    A common trap is: Using debounce when continuous feedback is required.
...
// A1. Debounce waits until calls stop before running a function.
//
// Q2. What is the runtime rule?
// A2. each call resets the timer
//
// Q3. What bug does this cause in real projects?
// A3. Using debounce when continuous feedback is required.
//
// Q4. How do you avoid the bug?
// A4. Use for search input, resize-end, and autosave-after-typing.
//
...
//   Back : Debounce waits until calls stop before running a function.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : each call resets the timer
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Using debounce when continuous feedback is required.
//
// Flashcard 4:
...
//
// For "Debounce", practice these levels:
//
// Level 1 — Definition:
//   Debounce waits until calls stop before running a function.
//
...
//    In this case, the rule is: each call resets the timer
//    That is why the common bug happens: Using debounce when continuous feedback is required."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Debounce. Predict the output and
...
//   3. The exact rule: each call resets the timer
//   4. The output or state change.
//   5. The bug risk: Using debounce when continuous feedback is required.
//
