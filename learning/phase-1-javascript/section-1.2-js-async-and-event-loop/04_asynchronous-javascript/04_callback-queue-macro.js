// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  04_callback-queue-macro.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Callback Queue (macro)
//
...
// Macrotasks include timers, I/O callbacks, events, and message callbacks.
//
...
//
// Runtime rule:
//   the loop handles one macrotask after microtasks are empty
//
// Practical rule:
//   Timers wait behind sync work and microtasks.
//
// Common trap:
//   Long macrotasks block user interaction and later callbacks.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/04_callback-queue-macro.js"

console.log("A sync start");

const job = Promise.resolve("Callback Queue (macro)");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Callback Queue (macro)");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Callback Queue (macro)");
  steps.push("2. Apply rule: the loop handles one macrotask after microtasks are empty");
  steps.push("3. Avoid trap: Long macrotasks block user interaction and later callbacks.");
...
//
// Interview framing:
//   "I care about Callback Queue (macro) because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Long macrotasks block user interaction and later callbacks.",
    rule: "the loop handles one macrotask after microtasks are empty",
    fix: "Timers wait behind sync work and microtasks.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Callback Queue (macro)", answer like this:
//
//   "At a high level, Macrotasks include timers, I/O callbacks, events, and message callbacks.
//    The important runtime rule is: the loop handles one macrotask after microtasks are empty
//    A common trap is: Long macrotasks block user interaction and later callbacks.
...
// A1. Macrotasks include timers, I/O callbacks, events, and message callbacks.
//
// Q2. What is the runtime rule?
// A2. the loop handles one macrotask after microtasks are empty
//
// Q3. What bug does this cause in real projects?
// A3. Long macrotasks block user interaction and later callbacks.
//
// Q4. How do you avoid the bug?
// A4. Timers wait behind sync work and microtasks.
//
...
//
// Flashcard 1:
//   Front: What is Callback Queue (macro)?
//   Back : Macrotasks include timers, I/O callbacks, events, and message callbacks.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : the loop handles one macrotask after microtasks are empty
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Long macrotasks block user interaction and later callbacks.
//
// Flashcard 4:
...
//
// For "Callback Queue (macro)", practice these levels:
//
// Level 1 — Definition:
//   Macrotasks include timers, I/O callbacks, events, and message callbacks.
//
...
//    In this case, the rule is: the loop handles one macrotask after microtasks are empty
//    That is why the common bug happens: Long macrotasks block user interaction and later callbacks."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Callback Queue (macro). Predict the output and
...
//   3. The exact rule: the loop handles one macrotask after microtasks are empty
//   4. The output or state change.
//   5. The bug risk: Long macrotasks block user interaction and later callbacks.
//
...
//   Timers wait behind sync work and microtasks.
//
// NEXT TOPIC -> 05_microtask-queue-priority.js
