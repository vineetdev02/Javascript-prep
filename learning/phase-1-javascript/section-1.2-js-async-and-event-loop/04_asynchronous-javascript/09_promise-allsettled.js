// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  09_promise-allsettled.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Promise.allSettled
//
...
//
// Runtime rule:
//   it always fulfills with `{status, value/reason}` objects
//
// Practical rule:
//   Use it for dashboards, bulk jobs, and partial success UIs.
//
// Common trap:
//   Treating results as direct values without checking `status`.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/09_promise-allsettled.js"

console.log("A sync start");

const job = Promise.resolve("Promise.allSettled");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Promise.allSettled");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Promise.allSettled");
  steps.push("2. Apply rule: it always fulfills with `{status, value/reason}` objects");
  steps.push("3. Avoid trap: Treating results as direct values without checking `status`.");
...
//
// Interview framing:
//   "I care about Promise.allSettled because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Treating results as direct values without checking `status`.",
    rule: "it always fulfills with `{status, value/reason}` objects",
    fix: "Use it for dashboards, bulk jobs, and partial success UIs.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Promise.allSettled", answer like this:
//
//   "At a high level, Promise.allSettled waits for every promise and reports each result.
//    The important runtime rule is: it always fulfills with `{status, value/reason}` objects
//    A common trap is: Treating results as direct values without checking `status`.
...
// A1. Promise.allSettled waits for every promise and reports each result.
//
// Q2. What is the runtime rule?
// A2. it always fulfills with `{status, value/reason}` objects
//
// Q3. What bug does this cause in real projects?
// A3. Treating results as direct values without checking `status`.
//
// Q4. How do you avoid the bug?
// A4. Use it for dashboards, bulk jobs, and partial success UIs.
//
...
//
// Flashcard 1:
//   Front: What is Promise.allSettled?
//   Back : Promise.allSettled waits for every promise and reports each result.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : it always fulfills with `{status, value/reason}` objects
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Treating results as direct values without checking `status`.
//
// Flashcard 4:
...
//
// For "Promise.allSettled", practice these levels:
//
// Level 1 — Definition:
//   Promise.allSettled waits for every promise and reports each result.
//
...
//    In this case, the rule is: it always fulfills with `{status, value/reason}` objects
//    That is why the common bug happens: Treating results as direct values without checking `status`."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Promise.allSettled. Predict the output and
...
//   3. The exact rule: it always fulfills with `{status, value/reason}` objects
//   4. The output or state change.
//   5. The bug risk: Treating results as direct values without checking `status`.
//
