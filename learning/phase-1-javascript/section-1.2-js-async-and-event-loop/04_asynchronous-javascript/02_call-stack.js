// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  02_call-stack.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Call Stack
//
...
//
// Call Stack:
// The call stack tracks currently executing function calls in LIFO order.
//
...
//
// Practical rule:
//   Read stack traces from top error location down through callers.
//
// Common trap:
//   Recursive functions without a base case overflow the stack.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/02_call-stack.js"

function placeOrder(orderId) {
  console.log("1. placeOrder pushed");
  return validateOrder(orderId);
}

function validateOrder(orderId) {
  console.log("2. validateOrder pushed");
  return chargePayment(orderId);
}

function chargePayment(orderId) {
  console.log("3. chargePayment pushed");
  return createReceipt(orderId);
}

function createReceipt(orderId) {
  console.log("4. createReceipt pushed");
  return { orderId, status: "paid" };
}

console.log(placeOrder("ORD-101"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Call Stack");
  steps.push("2. Apply rule: each function call pushes a frame; each return pops it");
  steps.push("3. Avoid trap: Recursive functions without a base case overflow the stack.");
...
//
// Interview framing:
//   "I care about Call Stack because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Recursive functions without a base case overflow the stack.",
    rule: "each function call pushes a frame; each return pops it",
    fix: "Read stack traces from top error location down through callers.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Call Stack", answer like this:
//
//   "At a high level, The call stack tracks currently executing function calls in LIFO order.
//    The important runtime rule is: each function call pushes a frame; each return pops it
//    A common trap is: Recursive functions without a base case overflow the stack.
...
// A1. The call stack tracks currently executing function calls in LIFO order.
//
// Q2. What is the runtime rule?
// A2. each function call pushes a frame; each return pops it
//
// Q3. What bug does this cause in real projects?
// A3. Recursive functions without a base case overflow the stack.
//
// Q4. How do you avoid the bug?
// A4. Read stack traces from top error location down through callers.
//
...
//
// Flashcard 1:
//   Front: What is Call Stack?
//   Back : The call stack tracks currently executing function calls in LIFO order.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : each function call pushes a frame; each return pops it
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Recursive functions without a base case overflow the stack.
//
// Flashcard 4:
...
//
// For "Call Stack", practice these levels:
//
// Level 1 — Definition:
//   The call stack tracks currently executing function calls in LIFO order.
//
...
//    In this case, the rule is: each function call pushes a frame; each return pops it
//    That is why the common bug happens: Recursive functions without a base case overflow the stack."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Call Stack. Predict the output and
...
//   3. The exact rule: each function call pushes a frame; each return pops it
//   4. The output or state change.
//   5. The bug risk: Recursive functions without a base case overflow the stack.
//
...
//   Read stack traces from top error location down through callers.
//
// NEXT TOPIC -> 03_web-apis-node-apis.js
