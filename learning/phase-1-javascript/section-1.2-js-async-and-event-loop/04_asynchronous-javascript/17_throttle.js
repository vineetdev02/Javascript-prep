// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  17_throttle.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Throttle
//
...
//
// Runtime rule:
//   calls inside the cooldown are ignored or scheduled depending implementation
//
// Practical rule:
//   Use for scroll, mousemove, and resize progress.
//
// Common trap:
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/17_throttle.js"

console.log("A sync start");

const job = Promise.resolve("Throttle");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Throttle");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Throttle");
  steps.push("2. Apply rule: calls inside the cooldown are ignored or scheduled depending implementation");
  steps.push("3. Avoid trap: Confusing throttle with debounce.");
...
//
// Interview framing:
//   "I care about Throttle because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Confusing throttle with debounce.",
    rule: "calls inside the cooldown are ignored or scheduled depending implementation",
    fix: "Use for scroll, mousemove, and resize progress.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Throttle", answer like this:
//
//   "At a high level, Throttle runs at most once per interval while calls continue.
//    The important runtime rule is: calls inside the cooldown are ignored or scheduled depending implementation
//    A common trap is: Confusing throttle with debounce.
...
// A1. Throttle runs at most once per interval while calls continue.
//
// Q2. What is the runtime rule?
// A2. calls inside the cooldown are ignored or scheduled depending implementation
//
// Q3. What bug does this cause in real projects?
// A3. Confusing throttle with debounce.
//
// Q4. How do you avoid the bug?
// A4. Use for scroll, mousemove, and resize progress.
//
...
//   Back : Throttle runs at most once per interval while calls continue.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : calls inside the cooldown are ignored or scheduled depending implementation
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Confusing throttle with debounce.
//
// Flashcard 4:
...
//
// For "Throttle", practice these levels:
//
// Level 1 — Definition:
//   Throttle runs at most once per interval while calls continue.
//
...
//    In this case, the rule is: calls inside the cooldown are ignored or scheduled depending implementation
//    That is why the common bug happens: Confusing throttle with debounce."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Throttle. Predict the output and
...
//   3. The exact rule: calls inside the cooldown are ignored or scheduled depending implementation
//   4. The output or state change.
//   5. The bug risk: Confusing throttle with debounce.
//
...
//   Use for scroll, mousemove, and resize progress.
//
// NEXT TOPIC -> 18_settimeout-vs-setinterval.js
