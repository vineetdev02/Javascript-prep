// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  03_web-apis-node-apis.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Web APIs / Node APIs
//
...
//
// Web APIs / Node APIs:
// Host environments provide async APIs like timers, DOM events, fetch, fs, and crypto.
//
...
//
// Runtime rule:
//   JS delegates async work to browser or Node APIs, then callbacks are queued
//
// Practical rule:
//   Separate ECMAScript language features from runtime APIs.
//
// Common trap:
//   Saying `setTimeout` is part of the JavaScript language itself.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/03_web-apis-node-apis.js"

console.log("A sync start");

const job = Promise.resolve("Web APIs / Node APIs");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Web APIs / Node APIs");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Web APIs / Node APIs");
  steps.push("2. Apply rule: JS delegates async work to browser or Node APIs, then callbacks are queued");
  steps.push("3. Avoid trap: Saying `setTimeout` is part of the JavaScript language itself.");
...
//
// Interview framing:
//   "I care about Web APIs / Node APIs because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Saying `setTimeout` is part of the JavaScript language itself.",
    rule: "JS delegates async work to browser or Node APIs, then callbacks are queued",
    fix: "Separate ECMAScript language features from runtime APIs.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Web APIs / Node APIs", answer like this:
//
//   "At a high level, Host environments provide async APIs like timers, DOM events, fetch, fs, and crypto.
//    The important runtime rule is: JS delegates async work to browser or Node APIs, then callbacks are queued
//    A common trap is: Saying `setTimeout` is part of the JavaScript language itself.
...
// A1. Host environments provide async APIs like timers, DOM events, fetch, fs, and crypto.
//
// Q2. What is the runtime rule?
// A2. JS delegates async work to browser or Node APIs, then callbacks are queued
//
// Q3. What bug does this cause in real projects?
// A3. Saying `setTimeout` is part of the JavaScript language itself.
//
// Q4. How do you avoid the bug?
// A4. Separate ECMAScript language features from runtime APIs.
//
...
//
// Flashcard 1:
//   Front: What is Web APIs / Node APIs?
//   Back : Host environments provide async APIs like timers, DOM events, fetch, fs, and crypto.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : JS delegates async work to browser or Node APIs, then callbacks are queued
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Saying `setTimeout` is part of the JavaScript language itself.
//
// Flashcard 4:
...
//
// For "Web APIs / Node APIs", practice these levels:
//
// Level 1 — Definition:
//   Host environments provide async APIs like timers, DOM events, fetch, fs, and crypto.
//
...
//    In this case, the rule is: JS delegates async work to browser or Node APIs, then callbacks are queued
//    That is why the common bug happens: Saying `setTimeout` is part of the JavaScript language itself."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Web APIs / Node APIs. Predict the output and
...
//   3. The exact rule: JS delegates async work to browser or Node APIs, then callbacks are queued
//   4. The output or state change.
//   5. The bug risk: Saying `setTimeout` is part of the JavaScript language itself.
//
...
//   Separate ECMAScript language features from runtime APIs.
//
// NEXT TOPIC -> 04_callback-queue-macro.js
