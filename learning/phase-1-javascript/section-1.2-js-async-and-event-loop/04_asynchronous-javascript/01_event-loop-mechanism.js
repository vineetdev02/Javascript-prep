// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  01_event-loop-mechanism.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Event Loop mechanism
//
...
// The event loop coordinates the call stack, microtasks, macrotasks, and rendering.
//
...
//
// Practical rule:
//   Predict output by writing down stack -> microtask queue -> macrotask queue.
//
// Common trap:
//   Thinking `setTimeout(fn, 0)` means immediate execution.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/01_event-loop-mechanism.js"

console.log("A sync start");

const job = Promise.resolve("Event Loop mechanism");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Event Loop mechanism");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Event Loop mechanism");
  steps.push("2. Apply rule: sync code runs first, then microtasks drain, then one macrotask runs");
  steps.push("3. Avoid trap: Thinking `setTimeout(fn, 0)` means immediate execution.");
...
//
// Interview framing:
//   "I care about Event Loop mechanism because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Thinking `setTimeout(fn, 0)` means immediate execution.",
    rule: "sync code runs first, then microtasks drain, then one macrotask runs",
    fix: "Predict output by writing down stack -> microtask queue -> macrotask queue.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Event Loop mechanism", answer like this:
//
//   "At a high level, The event loop coordinates the call stack, microtasks, macrotasks, and rendering.
//    The important runtime rule is: sync code runs first, then microtasks drain, then one macrotask runs
//    A common trap is: Thinking `setTimeout(fn, 0)` means immediate execution.
...
// A1. The event loop coordinates the call stack, microtasks, macrotasks, and rendering.
//
// Q2. What is the runtime rule?
// A2. sync code runs first, then microtasks drain, then one macrotask runs
//
// Q3. What bug does this cause in real projects?
// A3. Thinking `setTimeout(fn, 0)` means immediate execution.
//
// Q4. How do you avoid the bug?
// A4. Predict output by writing down stack -> microtask queue -> macrotask queue.
//
...
//
// Flashcard 1:
//   Front: What is Event Loop mechanism?
//   Back : The event loop coordinates the call stack, microtasks, macrotasks, and rendering.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : sync code runs first, then microtasks drain, then one macrotask runs
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Thinking `setTimeout(fn, 0)` means immediate execution.
//
// Flashcard 4:
...
//
// For "Event Loop mechanism", practice these levels:
//
// Level 1 — Definition:
//   The event loop coordinates the call stack, microtasks, macrotasks, and rendering.
//
...
//    In this case, the rule is: sync code runs first, then microtasks drain, then one macrotask runs
//    That is why the common bug happens: Thinking `setTimeout(fn, 0)` means immediate execution."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Event Loop mechanism. Predict the output and
...
//   3. The exact rule: sync code runs first, then microtasks drain, then one macrotask runs
//   4. The output or state change.
//   5. The bug risk: Thinking `setTimeout(fn, 0)` means immediate execution.
//
