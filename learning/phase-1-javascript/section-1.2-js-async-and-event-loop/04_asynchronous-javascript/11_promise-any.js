// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  11_promise-any.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Promise.any
//
...
//
// Promise.any:
// Promise.any fulfills with the first successful promise and ignores rejections until all fail.
//
...
//
// Runtime rule:
//   rejects with AggregateError only when every input rejects
//
// Practical rule:
//   Use it for mirror endpoints or fallback providers.
//
// Common trap:
//   Confusing it with race, which fails on first rejection.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/11_promise-any.js"

console.log("A sync start");

const job = Promise.resolve("Promise.any");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Promise.any");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Promise.any");
  steps.push("2. Apply rule: rejects with AggregateError only when every input rejects");
  steps.push("3. Avoid trap: Confusing it with race, which fails on first rejection.");
...
//
// Interview framing:
//   "I care about Promise.any because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Confusing it with race, which fails on first rejection.",
    rule: "rejects with AggregateError only when every input rejects",
    fix: "Use it for mirror endpoints or fallback providers.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Promise.any", answer like this:
//
//   "At a high level, Promise.any fulfills with the first successful promise and ignores rejections until all fail.
//    The important runtime rule is: rejects with AggregateError only when every input rejects
//    A common trap is: Confusing it with race, which fails on first rejection.
...
// A1. Promise.any fulfills with the first successful promise and ignores rejections until all fail.
//
// Q2. What is the runtime rule?
// A2. rejects with AggregateError only when every input rejects
//
// Q3. What bug does this cause in real projects?
// A3. Confusing it with race, which fails on first rejection.
//
// Q4. How do you avoid the bug?
// A4. Use it for mirror endpoints or fallback providers.
//
...
//
// Flashcard 1:
//   Front: What is Promise.any?
//   Back : Promise.any fulfills with the first successful promise and ignores rejections until all fail.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : rejects with AggregateError only when every input rejects
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Confusing it with race, which fails on first rejection.
//
// Flashcard 4:
...
//
// For "Promise.any", practice these levels:
//
// Level 1 — Definition:
//   Promise.any fulfills with the first successful promise and ignores rejections until all fail.
//
...
//    In this case, the rule is: rejects with AggregateError only when every input rejects
//    That is why the common bug happens: Confusing it with race, which fails on first rejection."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Promise.any. Predict the output and
...
//   3. The exact rule: rejects with AggregateError only when every input rejects
//   4. The output or state change.
//   5. The bug risk: Confusing it with race, which fails on first rejection.
//
...
//   Use it for mirror endpoints or fallback providers.
//
// NEXT TOPIC -> 12_async-await-syntax.js
