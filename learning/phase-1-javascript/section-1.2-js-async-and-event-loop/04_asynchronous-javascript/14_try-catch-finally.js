// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  14_try-catch-finally.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: try/catch/finally
//
...
//
// Runtime rule:
//   try runs risky code, catch handles errors, finally always runs
//
// Practical rule:
//   Use finally for cleanup, locks, loading flags, and resources.
//
// Common trap:
//   Returning from finally and accidentally overriding the result.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/14_try-catch-finally.js"

console.log("A sync start");

const job = Promise.resolve("try/catch/finally");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for try/catch/finally");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: try/catch/finally");
  steps.push("2. Apply rule: try runs risky code, catch handles errors, finally always runs");
  steps.push("3. Avoid trap: Returning from finally and accidentally overriding the result.");
...
//
// Interview framing:
//   "I care about try/catch/finally because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Returning from finally and accidentally overriding the result.",
    rule: "try runs risky code, catch handles errors, finally always runs",
    fix: "Use finally for cleanup, locks, loading flags, and resources.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "try/catch/finally", answer like this:
//
//   "At a high level, finally runs cleanup after success or failure.
//    The important runtime rule is: try runs risky code, catch handles errors, finally always runs
//    A common trap is: Returning from finally and accidentally overriding the result.
...
// A1. finally runs cleanup after success or failure.
//
// Q2. What is the runtime rule?
// A2. try runs risky code, catch handles errors, finally always runs
//
// Q3. What bug does this cause in real projects?
// A3. Returning from finally and accidentally overriding the result.
//
// Q4. How do you avoid the bug?
// A4. Use finally for cleanup, locks, loading flags, and resources.
//
...
//
// Flashcard 1:
//   Front: What is try/catch/finally?
//   Back : finally runs cleanup after success or failure.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : try runs risky code, catch handles errors, finally always runs
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Returning from finally and accidentally overriding the result.
//
// Flashcard 4:
...
//
// For "try/catch/finally", practice these levels:
//
// Level 1 — Definition:
//   finally runs cleanup after success or failure.
//
...
//    In this case, the rule is: try runs risky code, catch handles errors, finally always runs
//    That is why the common bug happens: Returning from finally and accidentally overriding the result."
//
...
//
// Practice prompt:
//   "Here is a code snippet using try/catch/finally. Predict the output and
...
//   3. The exact rule: try runs risky code, catch handles errors, finally always runs
//   4. The output or state change.
//   5. The bug risk: Returning from finally and accidentally overriding the result.
//
...
//   Use finally for cleanup, locks, loading flags, and resources.
//
// NEXT TOPIC -> 15_callback-hell.js
