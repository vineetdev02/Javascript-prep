// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  18_settimeout-vs-setinterval.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: setTimeout vs setInterval
//
...
//
// Runtime rule:
//   recursive setTimeout can avoid overlapping async work
//
// Practical rule:
//   Use clearTimeout/clearInterval to avoid leaks.
//
// Common trap:
//   Letting intervals run after a component/page is gone.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/18_settimeout-vs-setinterval.js"

console.log("A sync start");

const job = Promise.resolve("setTimeout vs setInterval");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for setTimeout vs setInterval");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: setTimeout vs setInterval");
  steps.push("2. Apply rule: recursive setTimeout can avoid overlapping async work");
  steps.push("3. Avoid trap: Letting intervals run after a component/page is gone.");
...
//
// Interview framing:
//   "I care about setTimeout vs setInterval because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Letting intervals run after a component/page is gone.",
    rule: "recursive setTimeout can avoid overlapping async work",
    fix: "Use clearTimeout/clearInterval to avoid leaks.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "setTimeout vs setInterval", answer like this:
//
//   "At a high level, setTimeout runs once; setInterval repeats until cleared.
//    The important runtime rule is: recursive setTimeout can avoid overlapping async work
//    A common trap is: Letting intervals run after a component/page is gone.
...
// A1. setTimeout runs once; setInterval repeats until cleared.
//
// Q2. What is the runtime rule?
// A2. recursive setTimeout can avoid overlapping async work
//
// Q3. What bug does this cause in real projects?
// A3. Letting intervals run after a component/page is gone.
//
// Q4. How do you avoid the bug?
// A4. Use clearTimeout/clearInterval to avoid leaks.
//
...
const summary = {
  topic: "setTimeout vs setInterval",
  keyword: "timer scheduling",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is setTimeout vs setInterval?
//   Back : setTimeout runs once; setInterval repeats until cleared.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : recursive setTimeout can avoid overlapping async work
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Letting intervals run after a component/page is gone.
//
// Flashcard 4:
...
//
// For "setTimeout vs setInterval", practice these levels:
//
// Level 1 — Definition:
//   setTimeout runs once; setInterval repeats until cleared.
//
...
//    In this case, the rule is: recursive setTimeout can avoid overlapping async work
//    That is why the common bug happens: Letting intervals run after a component/page is gone."
//
...
//
// Practice prompt:
//   "Here is a code snippet using setTimeout vs setInterval. Predict the output and
...
//   3. The exact rule: recursive setTimeout can avoid overlapping async work
//   4. The output or state change.
//   5. The bug risk: Letting intervals run after a component/page is gone.
//
