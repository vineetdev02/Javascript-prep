// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  07_promise-chaining.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Promise chaining
//
...
//
// Promise chaining:
// Each `.then()` returns a new promise whose value depends on return/throw.
//
...
//
// Runtime rule:
//   return passes value forward; throw jumps to rejection path
//
// Practical rule:
//   Always return inner promises from `.then()` callbacks.
//
// Common trap:
//   Forgetting `return`, causing the next step to receive undefined.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/07_promise-chaining.js"

console.log("A sync start");

const job = Promise.resolve("Promise chaining");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Promise chaining");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Promise chaining");
  steps.push("2. Apply rule: return passes value forward; throw jumps to rejection path");
  steps.push("3. Avoid trap: Forgetting `return`, causing the next step to receive undefined.");
...
//
// Interview framing:
//   "I care about Promise chaining because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Forgetting `return`, causing the next step to receive undefined.",
    rule: "return passes value forward; throw jumps to rejection path",
    fix: "Always return inner promises from `.then()` callbacks.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Promise chaining", answer like this:
//
//   "At a high level, Each `.then()` returns a new promise whose value depends on return/throw.
//    The important runtime rule is: return passes value forward; throw jumps to rejection path
//    A common trap is: Forgetting `return`, causing the next step to receive undefined.
...
// A1. Each `.then()` returns a new promise whose value depends on return/throw.
//
// Q2. What is the runtime rule?
// A2. return passes value forward; throw jumps to rejection path
//
// Q3. What bug does this cause in real projects?
// A3. Forgetting `return`, causing the next step to receive undefined.
//
// Q4. How do you avoid the bug?
// A4. Always return inner promises from `.then()` callbacks.
//
...
//   Back : Each `.then()` returns a new promise whose value depends on return/throw.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : return passes value forward; throw jumps to rejection path
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Forgetting `return`, causing the next step to receive undefined.
//
// Flashcard 4:
...
//
// For "Promise chaining", practice these levels:
//
// Level 1 — Definition:
//   Each `.then()` returns a new promise whose value depends on return/throw.
//
...
//    In this case, the rule is: return passes value forward; throw jumps to rejection path
//    That is why the common bug happens: Forgetting `return`, causing the next step to receive undefined."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Promise chaining. Predict the output and
...
//   3. The exact rule: return passes value forward; throw jumps to rejection path
//   4. The output or state change.
//   5. The bug risk: Forgetting `return`, causing the next step to receive undefined.
//
