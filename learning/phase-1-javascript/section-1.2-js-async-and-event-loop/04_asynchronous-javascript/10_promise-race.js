// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  10_promise-race.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Promise.race
//
...
//
// Promise.race:
// Promise.race settles with the first input promise that settles.
//
...
//
// Runtime rule:
//   first fulfillment or first rejection wins
//
// Practical rule:
//   Use it for timeouts and fastest-source scenarios.
//
// Common trap:
//   Assuming losers are automatically cancelled.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/10_promise-race.js"

console.log("A sync start");

const job = Promise.resolve("Promise.race");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Promise.race");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Promise.race");
  steps.push("2. Apply rule: first fulfillment or first rejection wins");
  steps.push("3. Avoid trap: Assuming losers are automatically cancelled.");
...
//
// Interview framing:
//   "I care about Promise.race because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Assuming losers are automatically cancelled.",
    rule: "first fulfillment or first rejection wins",
    fix: "Use it for timeouts and fastest-source scenarios.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Promise.race", answer like this:
//
//   "At a high level, Promise.race settles with the first input promise that settles.
//    The important runtime rule is: first fulfillment or first rejection wins
//    A common trap is: Assuming losers are automatically cancelled.
...
// A1. Promise.race settles with the first input promise that settles.
//
// Q2. What is the runtime rule?
// A2. first fulfillment or first rejection wins
//
// Q3. What bug does this cause in real projects?
// A3. Assuming losers are automatically cancelled.
//
// Q4. How do you avoid the bug?
// A4. Use it for timeouts and fastest-source scenarios.
//
...
//
// Flashcard 1:
//   Front: What is Promise.race?
//   Back : Promise.race settles with the first input promise that settles.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : first fulfillment or first rejection wins
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Assuming losers are automatically cancelled.
//
// Flashcard 4:
...
//
// For "Promise.race", practice these levels:
//
// Level 1 — Definition:
//   Promise.race settles with the first input promise that settles.
//
...
//    In this case, the rule is: first fulfillment or first rejection wins
//    That is why the common bug happens: Assuming losers are automatically cancelled."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Promise.race. Predict the output and
...
//   3. The exact rule: first fulfillment or first rejection wins
//   4. The output or state change.
//   5. The bug risk: Assuming losers are automatically cancelled.
//
