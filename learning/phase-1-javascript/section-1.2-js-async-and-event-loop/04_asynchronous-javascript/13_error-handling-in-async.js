// ╔══════════════════════════════════════════════════════════════════╗
// ║   Asynchronous JavaScript  →  13_error-handling-in-async.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Error handling in async
//
...
// Async errors are promise rejections unless properly awaited/caught.
//
...
//
// Runtime rule:
//   throw inside async becomes rejected promise
//
// Practical rule:
//   Use `await` inside `try/catch` or attach `.catch()`.
//
// Common trap:
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.2-js-async-and-event-loop/04_asynchronous-javascript/13_error-handling-in-async.js"

console.log("A sync start");

const job = Promise.resolve("Error handling in async");
job.then((value) => console.log("C microtask:", value));

setTimeout(() => {
  console.log("D macrotask timer for Error handling in async");
}, 0);
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Error handling in async");
  steps.push("2. Apply rule: throw inside async becomes rejected promise");
  steps.push("3. Avoid trap: try/catch around an async call without `await`.");
...
//
// Interview framing:
//   "I care about Error handling in async because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "try/catch around an async call without `await`.",
    rule: "throw inside async becomes rejected promise",
    fix: "Use `await` inside `try/catch` or attach `.catch()`.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Error handling in async", answer like this:
//
//   "At a high level, Async errors are promise rejections unless properly awaited/caught.
//    The important runtime rule is: throw inside async becomes rejected promise
//    A common trap is: try/catch around an async call without `await`.
...
// A1. Async errors are promise rejections unless properly awaited/caught.
//
// Q2. What is the runtime rule?
// A2. throw inside async becomes rejected promise
//
// Q3. What bug does this cause in real projects?
// A3. try/catch around an async call without `await`.
//
// Q4. How do you avoid the bug?
// A4. Use `await` inside `try/catch` or attach `.catch()`.
//
...
const summary = {
  topic: "Error handling in async",
  keyword: "async rejection",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is Error handling in async?
//   Back : Async errors are promise rejections unless properly awaited/caught.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : throw inside async becomes rejected promise
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : try/catch around an async call without `await`.
//
// Flashcard 4:
...
//
// For "Error handling in async", practice these levels:
//
// Level 1 — Definition:
//   Async errors are promise rejections unless properly awaited/caught.
//
...
//    In this case, the rule is: throw inside async becomes rejected promise
//    That is why the common bug happens: try/catch around an async call without `await`."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Error handling in async. Predict the output and
...
//   3. The exact rule: throw inside async becomes rejected promise
//   4. The output or state change.
//   5. The bug risk: try/catch around an async call without `await`.
//
...
//   Use `await` inside `try/catch` or attach `.catch()`.
//
// NEXT TOPIC -> 14_try-catch-finally.js
