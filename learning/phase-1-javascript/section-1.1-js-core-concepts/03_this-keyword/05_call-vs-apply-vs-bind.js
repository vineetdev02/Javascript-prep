// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  05_call-vs-apply-vs-bind.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: call() vs apply() vs bind()
//
...
// call/apply invoke immediately; bind returns a new function with fixed `this`.
//
...
//
// Runtime rule:
//   `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//
// Practical rule:
//   Use call/apply for immediate invocation and bind for later callbacks.
//
// Common trap:
//   Forgetting bind does not run the function immediately.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/05_call-vs-apply-vs-bind.js"
...
console.log(showBinding.call(interviewUser, "call() vs apply() vs bind()"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: call() vs apply() vs bind()");
  steps.push("2. Apply rule: `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`");
  steps.push("3. Avoid trap: Forgetting bind does not run the function immediately.");
...
//
// Interview framing:
//   "I care about call() vs apply() vs bind() because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Forgetting bind does not run the function immediately.",
    rule: "`call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`",
    fix: "Use call/apply for immediate invocation and bind for later callbacks.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "call() vs apply() vs bind()", answer like this:
//
//   "At a high level, call/apply invoke immediately; bind returns a new function with fixed `this`.
//    The important runtime rule is: `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//    A common trap is: Forgetting bind does not run the function immediately.
...
// A1. call/apply invoke immediately; bind returns a new function with fixed `this`.
//
// Q2. What is the runtime rule?
// A2. `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//
// Q3. What bug does this cause in real projects?
// A3. Forgetting bind does not run the function immediately.
//
// Q4. How do you avoid the bug?
// A4. Use call/apply for immediate invocation and bind for later callbacks.
//
...
//
// Flashcard 1:
//   Front: What is call() vs apply() vs bind()?
//   Back : call/apply invoke immediately; bind returns a new function with fixed `this`.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Forgetting bind does not run the function immediately.
//
// Flashcard 4:
...
//
// For "call() vs apply() vs bind()", practice these levels:
//
// Level 1 — Definition:
//   call/apply invoke immediately; bind returns a new function with fixed `this`.
//
...
//    In this case, the rule is: `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//    That is why the common bug happens: Forgetting bind does not run the function immediately."
//
...
//
// Practice prompt:
//   "Here is a code snippet using call() vs apply() vs bind(). Predict the output and
...
//   3. The exact rule: `call(obj, a, b)`, `apply(obj, [a,b])`, `bind(obj, a)`
//   4. The output or state change.
//   5. The bug risk: Forgetting bind does not run the function immediately.
//
...
//   Use call/apply for immediate invocation and bind for later callbacks.
//
// NEXT TOPIC -> 06_explicit-vs-implicit-binding.js
