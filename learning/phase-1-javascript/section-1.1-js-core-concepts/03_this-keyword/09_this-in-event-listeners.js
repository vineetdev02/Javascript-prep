// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  09_this-in-event-listeners.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: this in event listeners
//
...
// Regular DOM listeners bind `this` to `event.currentTarget`; arrows do not.
//
...
//
// Runtime rule:
//   In regular addEventListener callbacks, `this === event.currentTarget`
//
// Practical rule:
//   Prefer `event.currentTarget` in modern code for clarity.
//
// Common trap:
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/09_this-in-event-listeners.js"
...
console.log(showBinding.call(interviewUser, "this in event listeners"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: this in event listeners");
  steps.push("2. Apply rule: In regular addEventListener callbacks, `this === event.currentTarget`");
  steps.push("3. Avoid trap: Confusing `event.target` with `event.currentTarget`.");
...
//
// Interview framing:
//   "I care about this in event listeners because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Confusing `event.target` with `event.currentTarget`.",
    rule: "In regular addEventListener callbacks, `this === event.currentTarget`",
    fix: "Prefer `event.currentTarget` in modern code for clarity.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "this in event listeners", answer like this:
//
//   "At a high level, Regular DOM listeners bind `this` to `event.currentTarget`; arrows do not.
//    The important runtime rule is: In regular addEventListener callbacks, `this === event.currentTarget`
//    A common trap is: Confusing `event.target` with `event.currentTarget`.
...
// A1. Regular DOM listeners bind `this` to `event.currentTarget`; arrows do not.
//
// Q2. What is the runtime rule?
// A2. In regular addEventListener callbacks, `this === event.currentTarget`
//
// Q3. What bug does this cause in real projects?
// A3. Confusing `event.target` with `event.currentTarget`.
//
// Q4. How do you avoid the bug?
// A4. Prefer `event.currentTarget` in modern code for clarity.
//
...
const summary = {
  topic: "this in event listeners",
  keyword: "event currentTarget",
  hasMentalModel: true,
  hasTrap: true,
};
...
//
// Flashcard 1:
//   Front: What is this in event listeners?
//   Back : Regular DOM listeners bind `this` to `event.currentTarget`; arrows do not.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : In regular addEventListener callbacks, `this === event.currentTarget`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Confusing `event.target` with `event.currentTarget`.
//
// Flashcard 4:
...
//
// For "this in event listeners", practice these levels:
//
// Level 1 — Definition:
//   Regular DOM listeners bind `this` to `event.currentTarget`; arrows do not.
//
...
//    In this case, the rule is: In regular addEventListener callbacks, `this === event.currentTarget`
//    That is why the common bug happens: Confusing `event.target` with `event.currentTarget`."
//
...
//
// Practice prompt:
//   "Here is a code snippet using this in event listeners. Predict the output and
...
//   3. The exact rule: In regular addEventListener callbacks, `this === event.currentTarget`
//   4. The output or state change.
//   5. The bug risk: Confusing `event.target` with `event.currentTarget`.
//
