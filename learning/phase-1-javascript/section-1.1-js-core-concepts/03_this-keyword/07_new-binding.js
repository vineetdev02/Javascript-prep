// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  07_new-binding.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: new binding
//
...
//
// new binding:
// `new` creates an object, links the prototype, binds `this`, and returns the object.
//
...
//
// Runtime rule:
//   `new Fn()` gives Fn a fresh object as `this`
//
// Practical rule:
//   Explain the four steps of `new` cleanly in interviews.
//
// Common trap:
//   Calling constructor functions without `new`.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/07_new-binding.js"
...
console.log(showBinding.call(interviewUser, "new binding"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: new binding");
  steps.push("2. Apply rule: `new Fn()` gives Fn a fresh object as `this`");
  steps.push("3. Avoid trap: Calling constructor functions without `new`.");
...
//
// Interview framing:
//   "I care about new binding because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Calling constructor functions without `new`.",
    rule: "`new Fn()` gives Fn a fresh object as `this`",
    fix: "Explain the four steps of `new` cleanly in interviews.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "new binding", answer like this:
//
//   "At a high level, `new` creates an object, links the prototype, binds `this`, and returns the object.
//    The important runtime rule is: `new Fn()` gives Fn a fresh object as `this`
//    A common trap is: Calling constructor functions without `new`.
...
// A1. `new` creates an object, links the prototype, binds `this`, and returns the object.
//
// Q2. What is the runtime rule?
// A2. `new Fn()` gives Fn a fresh object as `this`
//
// Q3. What bug does this cause in real projects?
// A3. Calling constructor functions without `new`.
//
// Q4. How do you avoid the bug?
// A4. Explain the four steps of `new` cleanly in interviews.
//
...
//
// Flashcard 1:
//   Front: What is new binding?
//   Back : `new` creates an object, links the prototype, binds `this`, and returns the object.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `new Fn()` gives Fn a fresh object as `this`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Calling constructor functions without `new`.
//
// Flashcard 4:
...
//
// For "new binding", practice these levels:
//
// Level 1 — Definition:
//   `new` creates an object, links the prototype, binds `this`, and returns the object.
//
...
//    In this case, the rule is: `new Fn()` gives Fn a fresh object as `this`
//    That is why the common bug happens: Calling constructor functions without `new`."
//
...
//
// Practice prompt:
//   "Here is a code snippet using new binding. Predict the output and
...
//   3. The exact rule: `new Fn()` gives Fn a fresh object as `this`
//   4. The output or state change.
//   5. The bug risk: Calling constructor functions without `new`.
//
...
//   Explain the four steps of `new` cleanly in interviews.
//
// NEXT TOPIC -> 08_losing-this-context.js
