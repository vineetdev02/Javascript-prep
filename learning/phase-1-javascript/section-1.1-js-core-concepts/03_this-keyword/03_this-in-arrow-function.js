// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  03_this-in-arrow-function.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: this in arrow function
//
...
// Arrow functions do not create their own `this`; they close over lexical `this`.
//
...
//
// Runtime rule:
//   Arrows are excellent inside callbacks but dangerous as object methods
//
// Practical rule:
//   Use arrows when you want surrounding `this`, not a fresh call-site `this`.
//
// Common trap:
//   Trying to change arrow `this` with call/apply/bind.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/03_this-in-arrow-function.js"
...
console.log(showBinding.call(interviewUser, "this in arrow function"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: this in arrow function");
  steps.push("2. Apply rule: Arrows are excellent inside callbacks but dangerous as object methods");
  steps.push("3. Avoid trap: Trying to change arrow `this` with call/apply/bind.");
...
//
// Interview framing:
//   "I care about this in arrow function because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Trying to change arrow `this` with call/apply/bind.",
    rule: "Arrows are excellent inside callbacks but dangerous as object methods",
    fix: "Use arrows when you want surrounding `this`, not a fresh call-site `this`.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "this in arrow function", answer like this:
//
//   "At a high level, Arrow functions do not create their own `this`; they close over lexical `this`.
//    The important runtime rule is: Arrows are excellent inside callbacks but dangerous as object methods
//    A common trap is: Trying to change arrow `this` with call/apply/bind.
...
// A1. Arrow functions do not create their own `this`; they close over lexical `this`.
//
// Q2. What is the runtime rule?
// A2. Arrows are excellent inside callbacks but dangerous as object methods
//
// Q3. What bug does this cause in real projects?
// A3. Trying to change arrow `this` with call/apply/bind.
//
// Q4. How do you avoid the bug?
// A4. Use arrows when you want surrounding `this`, not a fresh call-site `this`.
//
...
//
// Flashcard 1:
//   Front: What is this in arrow function?
//   Back : Arrow functions do not create their own `this`; they close over lexical `this`.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : Arrows are excellent inside callbacks but dangerous as object methods
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Trying to change arrow `this` with call/apply/bind.
//
// Flashcard 4:
...
//
// For "this in arrow function", practice these levels:
//
// Level 1 — Definition:
//   Arrow functions do not create their own `this`; they close over lexical `this`.
//
...
//    In this case, the rule is: Arrows are excellent inside callbacks but dangerous as object methods
//    That is why the common bug happens: Trying to change arrow `this` with call/apply/bind."
//
...
//
// Practice prompt:
//   "Here is a code snippet using this in arrow function. Predict the output and
...
//   3. The exact rule: Arrows are excellent inside callbacks but dangerous as object methods
//   4. The output or state change.
//   5. The bug risk: Trying to change arrow `this` with call/apply/bind.
//
...
//   Use arrows when you want surrounding `this`, not a fresh call-site `this`.
//
// NEXT TOPIC -> 04_this-in-class.js
