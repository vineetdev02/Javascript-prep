// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  04_this-in-class.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: this in class
//
...
//
// this in class:
// `this` in classes is the instance for instance methods and the constructor for static methods.
//
...
//
// Runtime rule:
//   Class methods are strict mode; detached methods get `undefined` as `this`
//
// Practical rule:
//   Prototype methods are shared; arrow fields are per-instance and already bound.
//
// Common trap:
//   Passing a class method as a callback without binding it.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/04_this-in-class.js"
...
console.log(showBinding.call(interviewUser, "this in class"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: this in class");
  steps.push("2. Apply rule: Class methods are strict mode; detached methods get `undefined` as `this`");
  steps.push("3. Avoid trap: Passing a class method as a callback without binding it.");
...
//
// Interview framing:
//   "I care about this in class because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Passing a class method as a callback without binding it.",
    rule: "Class methods are strict mode; detached methods get `undefined` as `this`",
    fix: "Prototype methods are shared; arrow fields are per-instance and already bound.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "this in class", answer like this:
//
//   "At a high level, `this` in classes is the instance for instance methods and the constructor for static methods.
//    The important runtime rule is: Class methods are strict mode; detached methods get `undefined` as `this`
//    A common trap is: Passing a class method as a callback without binding it.
...
// A1. `this` in classes is the instance for instance methods and the constructor for static methods.
//
// Q2. What is the runtime rule?
// A2. Class methods are strict mode; detached methods get `undefined` as `this`
//
// Q3. What bug does this cause in real projects?
// A3. Passing a class method as a callback without binding it.
//
// Q4. How do you avoid the bug?
// A4. Prototype methods are shared; arrow fields are per-instance and already bound.
//
...
//
// Flashcard 1:
//   Front: What is this in class?
//   Back : `this` in classes is the instance for instance methods and the constructor for static methods.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : Class methods are strict mode; detached methods get `undefined` as `this`
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Passing a class method as a callback without binding it.
//
// Flashcard 4:
...
//
// For "this in class", practice these levels:
//
// Level 1 — Definition:
//   `this` in classes is the instance for instance methods and the constructor for static methods.
//
...
//    In this case, the rule is: Class methods are strict mode; detached methods get `undefined` as `this`
//    That is why the common bug happens: Passing a class method as a callback without binding it."
//
...
//
// Practice prompt:
//   "Here is a code snippet using this in class. Predict the output and
...
//   3. The exact rule: Class methods are strict mode; detached methods get `undefined` as `this`
//   4. The output or state change.
//   5. The bug risk: Passing a class method as a callback without binding it.
//
...
//   Prototype methods are shared; arrow fields are per-instance and already bound.
//
// NEXT TOPIC -> 05_call-vs-apply-vs-bind.js
