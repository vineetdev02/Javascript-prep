// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  01_this-in-global-context.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: this in global context
//
...
// Global `this` depends on runtime and module system.
//
...
//
// Runtime rule:
//   `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//
// Practical rule:
//   Use `globalThis` when you need the real global object across runtimes.
//
// Common trap:
//   Assuming every environment behaves like browser script code.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/01_this-in-global-context.js"
...
console.log(showBinding.call(interviewUser, "this in global context"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: this in global context");
  steps.push("2. Apply rule: `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS");
  steps.push("3. Avoid trap: Assuming every environment behaves like browser script code.");
...
//
// Interview framing:
//   "I care about this in global context because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Assuming every environment behaves like browser script code.",
    rule: "`window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS",
    fix: "Use `globalThis` when you need the real global object across runtimes.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "this in global context", answer like this:
//
//   "At a high level, Global `this` depends on runtime and module system.
//    The important runtime rule is: `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//    A common trap is: Assuming every environment behaves like browser script code.
...
// A1. Global `this` depends on runtime and module system.
//
// Q2. What is the runtime rule?
// A2. `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//
// Q3. What bug does this cause in real projects?
// A3. Assuming every environment behaves like browser script code.
//
// Q4. How do you avoid the bug?
// A4. Use `globalThis` when you need the real global object across runtimes.
//
...
//
// Flashcard 1:
//   Front: What is this in global context?
//   Back : Global `this` depends on runtime and module system.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Assuming every environment behaves like browser script code.
//
// Flashcard 4:
...
//
// For "this in global context", practice these levels:
//
// Level 1 — Definition:
//   Global `this` depends on runtime and module system.
//
...
//    In this case, the rule is: `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//    That is why the common bug happens: Assuming every environment behaves like browser script code."
//
...
//
// Practice prompt:
//   "Here is a code snippet using this in global context. Predict the output and
...
//   3. The exact rule: `window` in classic browser scripts, `undefined` in ES modules, `module.exports` in Node CommonJS
//   4. The output or state change.
//   5. The bug risk: Assuming every environment behaves like browser script code.
//
...
//   Use `globalThis` when you need the real global object across runtimes.
//
// NEXT TOPIC -> 02_this-in-object-method.js
