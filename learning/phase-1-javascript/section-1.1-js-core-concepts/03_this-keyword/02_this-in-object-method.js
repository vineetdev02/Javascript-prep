// ╔══════════════════════════════════════════════════════════════════╗
// ║   this Keyword  →  02_this-in-object-method.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: this in object method
//
...
// A regular method call binds `this` to the object before the dot.
//
...
//
// Runtime rule:
//   `obj.method()` makes `this === obj` inside the method
//
// Practical rule:
//   Read the call site first. The object before the dot is the binding.
//
// Common trap:
//   Extracting a method into a variable removes the object before the dot.
//
...
// Predict the output.
// Then run:
//   node "learning/phase-1-javascript/section-1.1-js-core-concepts/03_this-keyword/02_this-in-object-method.js"
...
console.log(showBinding.call(interviewUser, "this in object method"));
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: this in object method");
  steps.push("2. Apply rule: `obj.method()` makes `this === obj` inside the method");
  steps.push("3. Avoid trap: Extracting a method into a variable removes the object before the dot.");
...
//
// Interview framing:
//   "I care about this in object method because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Extracting a method into a variable removes the object before the dot.",
    rule: "`obj.method()` makes `this === obj` inside the method",
    fix: "Read the call site first. The object before the dot is the binding.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "this in object method", answer like this:
//
//   "At a high level, A regular method call binds `this` to the object before the dot.
//    The important runtime rule is: `obj.method()` makes `this === obj` inside the method
//    A common trap is: Extracting a method into a variable removes the object before the dot.
...
// A1. A regular method call binds `this` to the object before the dot.
//
// Q2. What is the runtime rule?
// A2. `obj.method()` makes `this === obj` inside the method
//
// Q3. What bug does this cause in real projects?
// A3. Extracting a method into a variable removes the object before the dot.
//
// Q4. How do you avoid the bug?
// A4. Read the call site first. The object before the dot is the binding.
//
...
//
// Flashcard 1:
//   Front: What is this in object method?
//   Back : A regular method call binds `this` to the object before the dot.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : `obj.method()` makes `this === obj` inside the method
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Extracting a method into a variable removes the object before the dot.
//
// Flashcard 4:
...
//
// For "this in object method", practice these levels:
//
// Level 1 — Definition:
//   A regular method call binds `this` to the object before the dot.
//
...
//    In this case, the rule is: `obj.method()` makes `this === obj` inside the method
//    That is why the common bug happens: Extracting a method into a variable removes the object before the dot."
//
...
//
// Practice prompt:
//   "Here is a code snippet using this in object method. Predict the output and
...
//   3. The exact rule: `obj.method()` makes `this === obj` inside the method
//   4. The output or state change.
//   5. The bug risk: Extracting a method into a variable removes the object before the dot.
//
...
//   Read the call site first. The object before the dot is the binding.
//
// NEXT TOPIC -> 03_this-in-arrow-function.js
