// ╔══════════════════════════════════════════════════════════════════╗
// ║   Error Handling & Debugging  →  02_custom-error-classes.js
// ║   Level: Junior → Google/Apple Senior
// ╚══════════════════════════════════════════════════════════════════╝
//
// TARGET:
//   Crack interview questions around: Custom Error classes
//
...
// Custom errors represent domain-specific failures.
//
...
//
// Runtime rule:
//   extend Error, call super, set name, add metadata
//
// Practical rule:
//   Use custom errors for validation, auth, and business rules.
//
// Common trap:
...
function riskyOperation(shouldFail) {
  if (shouldFail) {
    throw new InterviewError("Custom Error classes failed", "DEMO_FAILURE");
  }
  return "Custom Error classes succeeded";
}
...
try {
  ...
  console.log("cleanup for Custom Error classes");
}
...
function predictionDrill() {
  ...
  steps.push("1. Identify topic: Custom Error classes");
  steps.push("2. Apply rule: extend Error, call super, set name, add metadata");
  steps.push("3. Avoid trap: Throwing plain strings instead of Error objects.");
...
//
// Interview framing:
//   "I care about Custom Error classes because it appears in real code when
...
function bugChecklist() {
  return {
    bug: "Throwing plain strings instead of Error objects.",
    rule: "extend Error, call super, set name, add metadata",
    fix: "Use custom errors for validation, auth, and business rules.",
  };
}
...
// ══════════════════════════════════════════════════════════════════
//
// When asked about "Custom Error classes", answer like this:
//
//   "At a high level, Custom errors represent domain-specific failures.
//    The important runtime rule is: extend Error, call super, set name, add metadata
//    A common trap is: Throwing plain strings instead of Error objects.
...
// A1. Custom errors represent domain-specific failures.
//
// Q2. What is the runtime rule?
// A2. extend Error, call super, set name, add metadata
//
// Q3. What bug does this cause in real projects?
// A3. Throwing plain strings instead of Error objects.
//
// Q4. How do you avoid the bug?
// A4. Use custom errors for validation, auth, and business rules.
//
...
//
// Flashcard 1:
//   Front: What is Custom Error classes?
//   Back : Custom errors represent domain-specific failures.
//
// Flashcard 2:
//   Front: What is the runtime rule?
//   Back : extend Error, call super, set name, add metadata
//
// Flashcard 3:
//   Front: What is the most common trap?
//   Back : Throwing plain strings instead of Error objects.
//
// Flashcard 4:
...
//
// For "Custom Error classes", practice these levels:
//
// Level 1 — Definition:
//   Custom errors represent domain-specific failures.
//
...
//    In this case, the rule is: extend Error, call super, set name, add metadata
//    That is why the common bug happens: Throwing plain strings instead of Error objects."
//
...
//
// Practice prompt:
//   "Here is a code snippet using Custom Error classes. Predict the output and
...
//   3. The exact rule: extend Error, call super, set name, add metadata
//   4. The output or state change.
//   5. The bug risk: Throwing plain strings instead of Error objects.
//
...
//   Use custom errors for validation, auth, and business rules.
//
// NEXT TOPIC -> 03_try-catch-scope.js
